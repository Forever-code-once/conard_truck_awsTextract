...
<?
	$mrr_save_preloader=0;
     ...
     $mrr_use_geotab=0;
     if(!isset($_GET['geotab_on']))
     {
     	$_GET['geotab_on']=$_SESSION['use_geotab_vs_pn'];
     }
     else
     {
     	$_SESSION['use_geotab_vs_pn']=(int) $_GET['geotab_on'];		
     }
     ...
     $mrr_use_geotab=$_SESSION['use_geotab_vs_pn'];
?>
<? include('functions_index_page.php') ?>
<?
	...
	*/
    ...
    $sql = "
                    ...
                    order by name_driver_last, name_driver_first
                ";
    $data_preplan_drivers = simple_query($sql);     //for preplanned driver function
...
<?
	...
	$mrr_maint_resx=mrr_get_equipment_maint_notice_warning_array('truck',"#ffcc00");
	$mrr_maint_truck_cntr=$mrr_maint_resx['num'];
	$mrr_maint_truck_arr=$mrr_maint_resx['arr'];
	$mrr_maint_truck_links=$mrr_maint_resx['links'];
	
	$mrr_maint_resy=mrr_get_equipment_maint_notice_warning_array('trailer',"#ffcc00");
	...
	$audio_alert_switch=0;
	$audio_alert_switch=(int) trim($defaultsarray['audio_alert_switch']);
	//if($_SERVER['REMOTE_ADDR'] == '70.90.229.29' || $_SERVER['HTTP_CF_CONNECTING_IP']=='70.90.229.29')		$audio_alert_switch=1;
	
	$peoplenet_geofencing_mph=(int) trim($defaultsarray['peoplenet_geofencing_mph']);
?>
<audio id='im_sound_affect_geo1' src='/sounds/FireTruck.mp3'></audio>
<audio id='im_sound_affect_geo' src='/sounds/Wet_Farts_SOUND_EFFECT.mp3'></audio>
<SCRIPT Language="Javascript">
		...
		setInterval( function() 
          {
              	...
              	<? if($mrr_use_geotab==1) { ?>
              		//mrr_fetch_sent_messages_home_geotab(); 	//new_geotab_messages
          	<? } else { ?>
          ...
          },(1000 * 60 * 1.00));		//1 minutes...1000=1 second
	});	
	
	function mrr_highlight_load_id(load_id, dispatch_id)
	{
		$(".mrr_load_"+load_id+"_disp_"+dispatch_id+"_styles").css( {"background-color":"red"});	
		$(".mrr_load_"+load_id+"_disp_"+dispatch_id+"_driver").css( {"color":"white"});		
		txt="";
		txt = txt + "<br>WARNING: ";
		txt = txt + "<a href='manage_load.php?load_id="+load_id+"&' target='_blank'>Load "+load_id+"</a> | ";
		txt = txt + "<a href='add_entry_truck.php?load_id="+load_id+"&id="+dispatch_id+"' target='_blank'>Dispatch "+dispatch_id+"</a>";
		txt = txt + " is running <span style='color:#cc0000;'><b>Late</b></span>!<br>";
		
		//$.prompt(txt);
		<? if($audio_alert_switch > 0)  { ?>$("#im_sound_affect_geo").get(0).play();<? } ?>
	}
	...
	function mrr_ignore_new_messages_geotab(id,userid)
	{			
		$.ajax({
			   type: "POST",
			   url: "ajax.php?cmd=mrr_geotab_update_incoming_message_ignore",
			   data: {
			   		"msg_id":id,
			   		"user_id":userid
			   		},
			   ...
			   success: function(xml) {
					mrr_fetch_sent_messages_home_geotab(); 		
			   }
		});
	}
	...
	function mrr_fetch_sent_messages_home_geotab()
	{	//mrr_geotab_messages_received
		$.ajax({
			   type: "POST",
			   url: "ajax.php?cmd=mrr_geotab_messages_sent",
			   data: {"truck_id": 0,
			   		"date_from": "",
			   		"date_to":  "",
			   		"truck_name": "",
			   ...
			   success: function(xml) {
					...
					$('#new_geotab_messages').html(mrrtab);
			   }
		});
	}
          ...
          <div class="page">
               <div class="container_left_part">
				...
				?>	
               	<? $page_timer_array[] = "Before mrr_conard_component sections: " . show_page_time(); ?>
					...
					?>
                    <?= mrr_conard_component_needs_appt() ?>
					<?= mrr_conard_component_driver_status() ?>
					<?= mrr_conard_component_driver_txt() ?>
               ...
               <div class="container_right_part">
				...
				//if($_SERVER['REMOTE_ADDR'] != '70.90.229.29') { 
					...
					//if($_SERVER['REMOTE_ADDR'] == '70.90.229.29')
					//{
						echo mrr_conard_component_calendar($_POST['use_mon'],$_POST['use_day'],$_POST['use_year'],$_POST['cal_mon'],$_POST['cal_day'],$_POST['cal_year']);
						$page_timer_array[] = "After mrr_conard_component_calendar: " . show_page_time();
					//}
					
					$use_geotab=$_SESSION['use_geotab_vs_pn'];
					if($use_geotab==1)
					{
						//echo mrr_conard_component_geotab_msgs();
					}
					else
					{
						//echo mrr_conard_component_peoplenet_msgs();
					}
					
					//echo mrr_conard_component_peoplenet_msgs();
					echo mrr_conard_component_fun_box();
					echo mrr_conard_component_geotab_msgs();
					...
					<?= mrr_conard_component_geofencing_msgs($use_geotab) ?>
				...
				<div id='avail_driver_summary' style='border: 1px solid #ffcc00; background-color:#cccccc; padding:10px; margin-top:10px; margin-bottom:10px;'></div>
...
<script type='text/javascript'>
	...
	function mrr_auto_update_preplan_marker(loadid)
	{
		myval=$('#preplan_marker_'+loadid+'_val').val();
		if(myval)
		{
     		$.ajax({
          			url: 'ajax.php?cmd=mrr_auto_update_preplan_marker', 
          			dataType: "xml",
          			type: "post",
          			data: {
          				"load_id": loadid,
          			...
          			cache: false,
          			success: function(xml) {
          				$.noticeAdd({text: "Success - Load "+loadid+" Preplan Marker updated."});				
          			}
          	});
     	}	
     	else
     		{
     			//$.prompt('looking for field preplan_marker'+loadid+'_val.');	
     		}
	}
    function mrr_auto_update_preplan_driver(loadid)
    {
        myval=parseInt($('#preplan_driver_'+loadid+'_val').val());
        if(myval > 0)
        {
            $.ajax({
                url: 'ajax.php?cmd=mrr_auto_update_preplan_driver',
                dataType: "xml",
                type: "post",
                data: {
                    "load_id": loadid,
                ...
                cache: false,
                success: function(xml) {
                    $.noticeAdd({text: "Success - Load "+loadid+" Preplan Driver updated."});
                }
            });
        }
        else
	...
	function toggle_avail_drivers()
	{
		$('#avail_driver_summary').toggle();
	}
	
	function mrr_get_avail_driver_summary()
	{
		$('#avail_driver_summary').html('Loading...');
		
		$.ajax({
     			url: 'ajax.php?cmd=mrr_get_avail_driver_summary', 
     			dataType: "xml",
     			type: "post",
     			data: {
     				"driver_id": 0
     			...
     			success: function(xml) {
     				...
     				mytext=""+$(xml).find('Disp').text()+"";    				
     				
     				$('#avail_driver_summary').html(mytext);				
     			}
     	});	
	}
	...
	function mrr_set_marquee()
	{
		...
		$('.marquee').marquee({
                    ...
                    css3easing: 'linear', 	//works when allowCss3Support is set to true - for full list see https://www.w3.org/TR/2013/WD-css3-transitions-20131119/#transition-timing-function                   
	...
	//var mrr_current_time_off_toggle=0;
	function mrr_swap_time_off() 
    {   //Time off switch from Drivers to Users/Admin
        /*
        if(mrr_current_time_off_toggle==0)
        {	//drivers
            $('.mrr_time_off_drivers').show();
            $('.mrr_time_off_users').hide();

            //mrr_current_time_off_toggle=1;
        }
        if(mrr_current_time_off_toggle==1)
        {	//admin/users
            $('.mrr_time_off_users').show();
            $('.mrr_time_off_drivers').hide();

            mrr_current_time_off_toggle=0;
        }
        */
        
        var mrr_switcher_label=$('#mrr_time_off_switcher').html();
        
        if(mrr_switcher_label=="Admin")
        {
            $('.mrr_time_off_users').show();
            $('.mrr_time_off_drivers').hide();

            $('#mrr_time_off_switcher').html("Drivers");
        }
        else 
        {
            $('.mrr_time_off_drivers').show();
            ...
            $('#mrr_time_off_switcher').html("Admin");
        }
    }
	...
	*/
	...
	function mrr_find_truck_locator_geotab(truckid,loadid)
	{
		$('#truck_local_'+loadid+'').html('Scanning GeoTab');	//this load only
	...
	$('.note_image').hover(
		function() {
			...
			has_load_id = $(this).attr('has_load_id');
			has_disp_id = $(this).attr('has_disp_id');
			...
			extra_html = "";
			 ...
			 $.ajax({
			   ...
			   success: function(data) {
			   		extra_html = "";
			   		if(has_load_flag == '1') extra_html += "<div class='format_alert'>Has Load (Next Load "+has_load_id+" | Next Dispatch "+has_disp_id+")</div>";
    ...
    $('.note_image2').hover(
        function() {
            pre_load_id = $(this).attr('pre_load_id');

            $('#note_holder2_'+pre_load_id).html("<img src='images/loader.gif'> Loading note...");

            $('#note_holder2_'+pre_load_id).show();
            
            extra_html = "";

            $.ajax({
                type: "POST",
                url: "ajax.php?cmd=load_notes_pre",
                data: {"pre_load_id":pre_load_id},
                success: function(data) {
                    //extra_html = "";
                    $('#note_holder2_'+pre_load_id).html(extra_html + data);
                }
            });
        },
        function() {
            $('#note_holder2_'+$(this).attr('pre_load_id')).hide();
        }
    );
		...
		if(msgtosend!="" && truck_id > 0 && sender_out==1)
		{
			...
			$.ajax({
			   ...
			   success: function(data) {			   		
			   		...
			   		$.noticeAdd({text: "Success - Message Sent."});	
			   }
			 });
		}
		else
	...
	function mrr_send_quick_msg_form_geotab(truck_id,load_id,log_id,sectioner)
	{		
		msgtosend=$('#truck_msg_'+truck_id+'_'+load_id+'_'+log_id+'_'+sectioner+'').val();
		...
		if(msgtosend!="" && truck_id > 0 && sender_out==1)
		{
			...
			$.ajax({
			   type: "POST",
			   url: "ajax.php?cmd=mrr_quick_message_form_sender_geotab",
			   data: {
			   		"truck_id":truck_id, 
			   		"disp_id":log_id,
			   		"load_id":load_id,
	...
	$('.truck_note_image_full').click(
		function() {
			...
			extra_html = "";
			 ...
			 $.ajax({
			   type: "POST",
			   url: "ajax.php?cmd=mrr_quick_message_form_display_geotab",
			   data: {
			   		"truck_id":truck_id, 
			   		"disp_id":log_id,
			   		"load_id":loadid,
	...
	function update_stop_arrival(load_handler_stop_id) 
	{
        ...
        funp="document.getElementById('popup_stop_date_projected').value='"+mrr_date+"'; document.getElementById('popup_stop_time_projected').value='"+mrr_time+"';";
		fun="document.getElementById('popup_stop_date_arrival').value='"+mrr_date+"'; document.getElementById('popup_stop_time_arrival').value='"+mrr_time+"';";
		
		fun1="document.getElementById('popup_stop_date_arrival').value='"+mrr_gmt_day+"'; document.getElementById('popup_stop_time_arrival').value='"+mrr_gmt_hrs+"';";
		fun2="document.getElementById('popup_stop_date_arrival').value='"+mrr_ast_day+"'; document.getElementById('popup_stop_time_arrival').value='"+mrr_ast_hrs+"';";
		fun3="document.getElementById('popup_stop_date_arrival').value='"+mrr_est_day+"'; document.getElementById('popup_stop_time_arrival').value='"+mrr_est_hrs+"';";
		fun4="document.getElementById('popup_stop_date_arrival').value='"+mrr_cst_day+"'; document.getElementById('popup_stop_time_arrival').value='"+mrr_cst_hrs+"';";
		fun5="document.getElementById('popup_stop_date_arrival').value='"+mrr_mst_day+"'; document.getElementById('popup_stop_time_arrival').value='"+mrr_mst_hrs+"';";
		fun6="document.getElementById('popup_stop_date_arrival').value='"+mrr_pst_day+"'; document.getElementById('popup_stop_time_arrival').value='"+mrr_pst_hrs+"';";
		
		//fun2="document.getElementById('popup_stop_date_arrival').value=''; document.getElementById('popup_stop_time_arrival').value='';";
		
		prompt_txt = "<table border='0' cellpaddoing='0' cellspacing='0' width='100%'>";
		
		prompt_txt = prompt_txt + "<tr><td colspan='3'>Enter the Arrival Date for this stop</td></tr>";
		
		prompt_txt = prompt_txt + "<tr>";
		prompt_txt = prompt_txt + 	"<td>Arrival Date</td>";
		prompt_txt = prompt_txt + 	"<td>Time</td>";
		prompt_txt = prompt_txt +  	"<td>&nbsp;</td>";	
		prompt_txt = prompt_txt + "</tr>";
		prompt_txt = prompt_txt + "<tr>";
		prompt_txt = prompt_txt + 	"<td><input name='popup_stop_date_arrival' id='popup_stop_date_arrival'></td>";
		prompt_txt = prompt_txt + 	"<td><input name='popup_stop_time_arrival' id='popup_stop_time_arrival'></td>";
		...
		prompt_txt = prompt_txt +  "<td>&nbsp;</td>";	
		prompt_txt = prompt_txt +  "<td>&nbsp;</td>";	
		prompt_txt = prompt_txt +  "<td><span class='mrr_link_like_on' title='click to put the current GMT date and time.' onClick=\""+fun+"\"><b>NOW</b></span></td>";
        ...
        prompt_txt = prompt_txt + "<tr>";
        prompt_txt = prompt_txt + 	"<td colspan='2'>...And/Or...</td>";
        prompt_txt = prompt_txt +  	"<td>&nbsp;</td>";
        prompt_txt = prompt_txt + "</tr>";

        prompt_txt = prompt_txt + "<tr>";
        prompt_txt = prompt_txt + 	"<td colspan='2'>Projected Availability Date</td>";
        prompt_txt = prompt_txt +  	"<td>&nbsp;</td>";
        prompt_txt = prompt_txt + "</tr>";
        prompt_txt = prompt_txt + "<tr>";
        prompt_txt = prompt_txt + 	"<td><input name='popup_stop_date_projected' id='popup_stop_date_projected'></td>";
        prompt_txt = prompt_txt + 	"<td><input name='popup_stop_time_projected' id='popup_stop_time_projected'></td>";
        prompt_txt = prompt_txt +  	"<td><span class='mrr_link_like_on' title='click to put the current GMT date and time.' onClick=\""+funp+"\"><b>NOW</b></span></td>";
        //prompt_txt = prompt_txt +  	"<td>Now: {Click on the time zone below.}</td>";		
        prompt_txt = prompt_txt + "</tr>";
						
		prompt_txt = prompt_txt + "</table>";
		
		function callbackfunct(v,m,f) {
			if(v) 
			{
			      if(f != undefined) 
			      { 	// && f.popup_stop_date_arrival != ''
					if(f.popup_stop_time_arrival.length != 5) 
					{
						//$.prompt("You must enter the arrival time for this stop or click Cancel");
						//return false;
					}
					
					if(f.popup_stop_date_arrival == '') 
					{
						//$.prompt("You must enter the arrival date for this stop or click Cancel");
						//return false;
					}
					
					if(f.popup_stop_grade_id == 0) {
						//$.prompt("Please Grade this stop based on the arrival date...");
						//return false;
					}
					...
					if((f.popup_stop_grade_id >= 10 || ( f.popup_stop_grade_id >= 1 && f.popup_stop_grade_id <= 4)) && f.popup_stop_grade_note.length < 4) {
					 ...
					 $.ajax({
					   	type: 'POST',
					   	url: 'ajax.php?cmd=update_stop_arrival',
					   	data: {
					   		stop_id:load_handler_stop_id,
					   		linedate_arrival:f.popup_stop_date_arrival,
					   		linedate_arrival_time:f.popup_stop_time_arrival,
                            projected_arrival:f.popup_stop_date_projected,
                            projected_arrival_time:f.popup_stop_time_projected,
				   		...
				   		cache:false,					   	
					   	error: function() {
					   			//$.prompt("<span class='alert'>ERROR:</span> There was an error updating the arrival date/time, please try again.");
							},
					  	success: function(xml)  {
					   		resulter=$(xml).find('rslt').text();
					   		if(resulter==1)
					   		{
					   			$.noticeAdd({text: "Success - Arrival Date and/or PTA set for Stop " + load_handler_stop_id});
					   			//$.prompt("Stop Arrival Date Set and Saved successfully");
					   		}
					   		else
					   		{
		...
		function loadedfunct() {
				$('#popup_stop_date_projected').datepicker();
                $('#popup_stop_date_arrival').datepicker();
				//$('#popup_stop_date_arrival').focus();
				$('#popup_stop_time_arrival').blur(simple_time_check);
                
				$('#popup_stop_grade_id').val(0);
				$('#popup_stop_grade_note').val('');

                $('#popup_stop_time_projected').blur(simple_time_check);
		}
					 ...
					 $.ajax({
					  	...
					  	success: function(xml)  {
					   		...
					   		if(resulter==1)
					   		{
					   			...
					   			$.noticeAdd({text: "Success - Date Completed set for Stop " + load_handler_stop_id});
					   		}
					   		else
					   		{
					   			$.prompt("<span class='alert'>ERROR:</span> "+$(xml).find('ErrorMsg').text()+".");	
	...
	*/
    ...
    function add_note(dispatch_id)
    {
        ...
        txt+="<div id='adding_notes_for_"+dispatch_id+"'></div><br>";
        ...
        $.prompt(txt, {
            ...
            loaded: function() {
                ...
                $.ajax({
                    url: "ajax.php?cmd=get_truck_notes",
                    dataType: "xml",
                    type: "post",
                    data: {
                        "dispatch_id": dispatch_id
                    ...
                    success: function(xml) {
                        ...
                        {
                            mrr_html=$(xml).find('DispHTML').text();
                        }

                        //$.noticeAdd({text: "Success - note was saved to dispatch " + dispatch_id});

                        $('#adding_notes_for_'+dispatch_id+'').html(''+mrr_html+'');
                    }
                });
	...
	function add_note_pre(pre_load_id) 
	{
		txt="";
		
		txt+="<div id='adding_notes_for_pre_"+pre_load_id+"'></div><br>";
		
		txt+="Enter your note for Load ID: "+pre_load_id+"<br>";
		txt+="<textarea style='width:400px;height:100px' name='new_note_pre' id='new_note_pre'></textarea><br><br>";
		$.prompt(txt, {
			buttons: {Okay: true, Cancel: false},
			loaded: function() {
				$('#new_note_pre').focus();
                ...
                $.ajax({
                    ...
                    data: {"pre_load_id":pre_load_id},
                    success: function(data) {
                        $('#adding_notes_for_pre_'+pre_load_id+'').html(data);
                    }
                });
				...
				if(v) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            */
                            $.noticeAdd({text: "Success - note was saved to load " + pre_load_id});
                        }
                    });
				}
			}
...
$rep=mrr_pn_request_form_processor(126373);
...
if($_SERVER['REMOTE_ADDR'] == '70.90.229.29' || $_SERVER['REMOTE_ADDR'] == '173.10.208.206' || 1 == 1) {
	echo "<pre>";
