...
<?
	...
	if(isset($_GET['dup_id'])) {
		...
		mrr_add_user_change_log($_SESSION['user_id'],0,0,0,0,$new_load_id,0,0,"Duplicated Load to  ".$new_load_id." info.");	//change log tracking... $_SESSION['user_id'],$cust,$driver,$truck,$trailer,$load,$disp,$stop,$notes	
	...
	if(isset($_GET['delid'])) {
		...
		$sql = "
			update geofence_hot_load_tracking
			set deleted = 1
...
<script type='text/javascript'>
	...
	function save_stop() {
		...
		if($('#pickup_eta').val() == '' || $('#pickup_eta_time').val() == '') {
			$.prompt("Appointment (ETA) Date and time are required fields.");
			return false;
		}
