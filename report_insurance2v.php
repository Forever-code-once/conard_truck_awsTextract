...
<?
	$use_title="Conard Insurance Report V2 (Full Year)";
?>
<? include('header.php') ?>
	...
	<tr>
		<td valign='top'>
          	<div style='text-align:left;margin:10px'>
          		<div class='section_heading'>Insurance Report V2</div>
          		Enter any date for the calendar YEAR you would like to build the insurance report for. <br>
          		For example, if you wanted <?=date("Y")?>, select any date during that year <br>
          		and the system will automatically generate the full year report.<br>
          		<a href='report_insurance.php' target='_blank'>Back to Monthly Insurance Report</a>
          		<br>
...
<?
	...
	if(isset($_POST['build_report'])) {
		...
		$date_start = strtotime(date("1/1/Y", strtotime($_POST['report_date'])));
		//$days_in_month = date("t", $date_start);
		//$date_end = strtotime(date("m/".$days_in_month."/Y", $date_start));
		$date_end = strtotime(date("12/31/Y", $date_start));
