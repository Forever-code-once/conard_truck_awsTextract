...
<? include('header.php') ?>
<div style='text-align:left;margin:10px;width:1220px;float:left' class='admin_menu1'>
	<div class='section_heading'>Days Available Report - Per Driver (Date Range)</div>
	<?
	...
	<?
		...
		function mrr_find_total_avail_days_in_months($start_date,$end_date)
		{
			$tot_days=0;
			$tot_months=0;
			$tot_billable=0;
			
			//echo "<br>Function Start ".$start_date." to ".$end_date.":";
			
			$use_date=strtotime($start_date);
			$days_in_month = date("t", $use_date);
			$tot_days+=$days_in_month;
			$tot_months++;
			
			//echo "<br> - - - This Month ".date("m/d/Y",$use_date)." = ".$days_in_month." days (Total =".$tot_days.").";
			
			$next_date=strtotime("+1 month",$use_date);
			while($next_date <= strtotime($end_date))
			{				
				$days_in_month = date("t", $next_date);
				$tot_days+=$days_in_month;
				$tot_months++;
				
				//echo "<br> - - - Next Month ".date("m/d/Y",$next_date)." = ".$days_in_month." days (Total =".$tot_days.").";
								
				$next_date=strtotime("+1 month",$next_date);	
			}	
			
			$res['tot_days']=$tot_days;
			$res['tot_months']=$tot_months;
			$res['tot_billable']=$tot_billable;
		...
		if(isset($_POST['build_report'])) 
		{
			...
			$res=mrr_find_total_avail_days_in_months($_POST['date_from'],$_POST['date_to']);
			//echo "<br>Total Days Queried=".$res['tot_days'].". (Billable Days=".$res['tot_billable'].", Months=".$res['tot_months'].")<br>";
			
			$days_in_month=$res['tot_days'];
			/*
			...
			*/
			...
			";			
			echo "</table><br>";
			echo "<br><b>Total Days Queried=".$res['tot_days']." (Months=".$res['tot_months'].").</b><br>";
		}
	?>
		</td>
	</tr>
