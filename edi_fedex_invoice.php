...
<?
	...
	if(isset($_GET['autorun'])) 
	{
		...
		// fedex customer id is 535
		// this EDI system was implemented after invoice number 19838, so only send
		// loads with the invoice number greater than that
		$sql = "
			SELECT * 
			
			FROM load_handler 
			WHERE customer_id = 535 
				AND deleted = 0 
				AND sicap_invoice_number >= 72000
		...
		while($row = mysqli_fetch_array($data)) 
		{
			...
			edi_fedex_create_invoice($row['id'],0,$mrr_output);
			echo " | done...<br>";
		}
	} 
	else 
	{
		...
		edi_fedex_create_invoice($load_id,$mrr_test,$mrr_output);
	}

	function edi_fedex_create_invoice($load_id,$test_file=0,$mrr_output=0) 
	{
		...
		{			
			...
			if($row['fedex_edi_invoice_mileage'] > 0)		$use_miles= $row['fedex_edi_invoice_mileage'];
			...
			if($test_file==0)
			{
     			...
     			if(send_fedex_edi('/FXFE/210/', $in_path."/out/", $out_file, $load_id,$mrr_output)) 
     			{
     				...
     				$sql = "
     					update load_handler set		
     						feded_edi_invoice_file='".sql_friendly("/out/".$out_file)."',	
     						fedex_edi_invoice_file_text='".sql_friendly($rval)."',		
     						linedate_edi_invoice_sent = now()
     					where id = '".sql_friendly($load_id)."'
     				";
     				simple_query($sql);	
     				
     				//mrr_add_fedex_edi_invoicing_log(0,$load_id,$sicap_invoice_number,$rval,1,0,0,(int) $use_miles);
     			}
     			else
     			{
