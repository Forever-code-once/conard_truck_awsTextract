...
<?
	$mrr_test=0;		//toggles output only of file to test its output...does nto send the file or create the existing one on our side...only outputs.
	if($_SERVER['REMOTE_ADDR'] == '70.90.229.29' || $_SERVER['HTTP_CF_CONNECTING_IP']=='70.90.229.29')		$mrr_test=1;	
	...
	if(isset($_GET['autorun'])) 
	{
		...
		// lynnco customer id is 2031
		// this EDI system was implemented after invoice number 82000, so only send
		// loads with the invoice number greater than that
		$sql = "
			SELECT * 
			
			FROM load_handler 
			WHERE customer_id = 2031
				AND deleted = 0 
				AND sicap_invoice_number >= 82000
		...
		while($row = mysqli_fetch_array($data)) 
		{
			...
			edi_lynnco_create_invoice($row['id']);
			echo " | done...<br>";
		}
	} 
	else 
	{
		...
		edi_lynnco_create_invoice($load_id,$mrr_test);
	}

	function edi_lynnco_create_invoice($load_id,$test_file=0) 
	{
		...
		{
			echo "LynnCo/Carlex EDI Invoice already sent on ".date("M j, Y h:i a", strtotime($row['linedate_edi_invoice_sent']));
			$process_210 = false;
		}
		...
		{			
			...
			{				
				...
				{					
					...
					if($stop_count==1)		$rval.=$mrr_rval;		//should get the first stop in the list.
				}
			}
			...
			if($row['lynnco_edi_invoice_mileage'] > 0)		$use_miles= $row['lynnco_edi_invoice_mileage'];
			...
			if($test_file==0)
			{
     			...
     			//$rslt = ftp_put($conn_id, "/FXFE/210/".$out_file, $in_path."/out/".$out_file, FTP_BINARY);
     			
     			//if(send_lynnco_edi('/FXFE/210/', $in_path."/out/", $out_file, $load_id)) 
     			if(send_lynnco_edi('/Inbound/', $in_path."/out/", $out_file, $load_id)) 
     			{
     				// upload was successful, remove that file from the server
     				//echo "<font color='green'>Successful Upload</font>";
     				$rslt = 1;
     				$msg = "LynnCo/Carlex EDI Invoice Sent Successfully";
     				
     				// log the upload
     				$sql = "
     					update load_handler set		
     						lynnco_edi_invoice_file='".sql_friendly("/out/".$out_file)."',	
     						lynnco_edi_invoice_file_text='".sql_friendly($rval)."',		
     						linedate_edi_invoice_sent = now()
     					where id = '".sql_friendly($load_id)."'
     				";
     				simple_query($sql);	
     				
     				mrr_add_lynnco_edi_invoicing_log(0,$load_id,$sicap_invoice_number,$rval,1,0,0,(int) $use_miles);
     			}
     			else
     			{
