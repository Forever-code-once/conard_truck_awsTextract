...
<?
	...
	if(isset($_POST['resend_items']) && isset($_POST['resend_id_list'])) {
		...
		echo "Resending the following FedEx Loads:<br>";
		foreach($_POST['resend_id_list'] as $value) 
		{
			echo "$value<br>";
			
			$load_id=(int) trim($value);
			mrr_add_fedex_edi_invoicing_log($_SESSION['user_id'],$load_id,0,'',0,0,1,0);
		}
		...
		$sql = "
			...
			where load_handler.deleted = 0
			    ...
			    and (
						...
						and load_handler.fedex_edi_input_file!=''
					)
        				".$search_date_range."
			     )				
	...
	<tr>
		<td colspan='10'>
			<center>
			<span class='section_heading'>FedEx EDI Invoiced Report</span>
			
			<br><br>Click to force the automatic <a href='edi_fedex_invoice.php?autorun=1' target='_blank'>FedEx EDI Invoicing</a> process to run right now. Otherwise, it will run again at 2:25AM on its own.<br><br>
			</center>
		</td>
	</tr>
