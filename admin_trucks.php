...
<?
	...
	{	//Added Nov 2015...MRR
		...
		$sql = "
			update trucks set
				...
				linedate_own_op_ins_exp = '0000-00-00 00:00:00',
				own_op_acc_ins_flag=0,
				linedate_own_op_acc_ins = '0000-00-00 00:00:00',
				linedate_own_op_acc_ins_exp = '0000-00-00 00:00:00',
				...
				geotab_device_id='',
				geotab_device_num='',
     			geotab_device_serial='',
     			geotab_device_type='',
     			rfid_tag_number='',
     			geotab_device_plan='',
     			geotab_last_odometer_reading='0.0000',
     			geotab_last_odometer_date='0000-00-00 00:00:00',
     			geotab_odometer_update_mode='0',
     			geotab_last_location_date='0000-00-00 00:00:00',
				vin='',
				ifta_number='',
				linedate_ifta='0000-00-00 00:00:00',
				linedate_cab_card='0000-00-00 00:00:00',
				use_pm_oil_report=0,
				use_pm_oil_report_valve=0,
                valve_miles_interval=0,
                valve_miles_last=0,
                valve_miles_last_date='0000-00-00 00:00:00',
                use_pm_oil_report_drain=0,
                drain_miles_interval=0,
                drain_miles_last=0,
                drain_miles_last_date='0000-00-00 00:00:00',
		...
		header("Location: admin_trucks.php?id=".$dup_id);
		die;
	}
	...
	{		
		...
		{	//redirect to existing truck if it already exists
         ...
         if(!isset($_POST['truck_photo']))		$_POST['truck_photo']="";
         ...
         if(isset($_FILES['mrr_import']['tmp_name']) && $_FILES['mrr_import']['tmp_name'] !="")
         {
              ...
              if($typer!="")
              {
                   ...
                   $mrr_msg.="<br><br>File Dest: ".$mrr_dir."truck_photo_".$_GET['id']."".$typer.".";
                   
                   if(move_uploaded_file ( $_FILES['mrr_import']['tmp_name'] , "".$mrr_dir."truck_photo_".$_GET['id']."".$typer.""))
                   {
                        $_POST['truck_photo']="truck_photo_".$_GET['id']."".$typer."";
		...
		if($_POST['linedate_own_op_ins_exp'] != '') 	$use_op_ins_due_exp = date("Y-m-d", strtotime($_POST['linedate_own_op_ins_exp']));
		$use_op_acc_ins_due='';
		$use_op_acc_ins_due_exp='';
		if($_POST['linedate_own_op_acc_ins'] != '') 	$use_op_acc_ins_due = date("Y-m-d", strtotime($_POST['linedate_own_op_acc_ins']));
		if($_POST['linedate_own_op_acc_ins_exp'] != '')	$use_op_acc_ins_due_exp = date("Y-m-d", strtotime($_POST['linedate_own_op_acc_ins_exp']));
		...
		$use_cab_exp='';
		$use_ifta_exp='';		
		if($_POST['linedate_ifta'] != '') 				$use_ifta_exp = date("Y-m-d", strtotime($_POST['linedate_ifta']));		
		if($_POST['linedate_cab_card'] != '') 			$use_cab_exp = date("Y-m-d", strtotime($_POST['linedate_cab_card']));	
		
		$_POST['geotab_device_id']= str_replace("B","b",$_POST['geotab_device_id']);
		$_POST['geotab_device_id']= str_replace("bb","bB",$_POST['geotab_device_id']);
				
		$update_log_oil=0;
		$update_log_avi=0;
        $update_log_drain=0;
        
		if($_POST['pm_miles_update_oil'] > 0)
        {
             $update_log_oil=1;
        }
		elseif($_POST['pm_miles_update_avi'] > 0)
        {
             $update_log_avi=1;
        }
        elseif($_POST['pm_miles_update_drain'] > 0)
        {
             $update_log_drain=1;
        }
		
		if($update_log_oil > 0 || $update_log_avi > 0 || $update_log_drain > 0) 
		{   //use oil dates as defaults...
		     $log_date=$_POST['pm_miles_last_date'];
		     $log_interval=$_POST['pm_miles_interval'];
		     $log_odom=$_POST['pm_miles_last_oil'];		     
		     $log_next_date="";
		     
		     if($update_log_avi > 0)
             {  //AVI used, so switch the date, odometer, and interval
                  $log_date=$_POST['valve_miles_last_date'];
                  $log_interval=$_POST['valve_miles_interval'];
                  $log_odom=$_POST['valve_miles_last'];                  
                  $log_next_date="";                 
             }
             
             if($update_log_drain > 0)
             {  //AVI used, so switch the date, odometer, and interval
                  $log_date=$_POST['drain_miles_last_date'];
                  $log_interval=$_POST['drain_miles_interval'];
                  $log_odom=$_POST['drain_miles_last'];
                  $log_next_date="";
             }
             ...
             $sql_log = "
		        ...
		        values 
		            ...
		            '',
		            '".($update_log_oil > 0 ? "1" : "0")."',
		            '".($update_log_avi > 0 ? "1" : "0")."',
		            '".($update_log_drain > 0 ? "1" : "0")."')
		     ";
		...
		$sql = "
			update trucks set
				...
				truck_color = '".sql_friendly($_POST['truck_color'])."',
				
				max_speed_cruise = '".sql_friendly((int) $_POST['max_speed_cruise'])."',
				max_speed_pedal = '".sql_friendly((int) $_POST['max_speed_pedal'])."',
				
				company_owned = '".(isset($_POST['company_owned']) ? '1' : '0') ."',
				company_admin_vehicle = '".(isset($_POST['company_admin_vehicle']) ? '1' : '0') ."',
				
				has_inverter = '".(isset($_POST['has_inverter']) ? '1' : '0')."',
				
				rfid_tag_number='".trim($_POST['rfid_tag_number'])."',
				tablet_imei='".trim($_POST['tablet_imei'])."',
				zenduit_drid='".trim($_POST['zenduit_drid'])."',
				
				truck_is_sold = '".(isset($_POST['truck_is_sold']) ? '1' : '0')."',
				...
				linedate_cab_card = '".($use_cab_exp == '' ? '0000-00-00' : sql_friendly($use_cab_exp))."',
				
				linedate_ifta = '".($use_ifta_exp == '' ? '0000-00-00' : sql_friendly($use_ifta_exp))."',
				linedate_own_op_ins = '".($use_op_ins_due == '' ? '0000-00-00' : sql_friendly($use_op_ins_due))."',
				linedate_own_op_ins_exp = '".($use_op_ins_due_exp == '' ? '0000-00-00' : sql_friendly($use_op_ins_due_exp))."',
				...
				linedate_own_op_acc_ins_exp = '".($use_op_acc_ins_due_exp == '' ? '0000-00-00' : sql_friendly($use_op_acc_ins_due_exp))."',
				...
				ifta_number = '".sql_friendly(trim($_POST['ifta_number']))."',
				...
				use_pm_oil_report_valve='".(isset($_POST['use_pm_oil_report_valve']) ? 1 : 0)."',
				valve_miles_interval='".sql_friendly(money_strip($_POST['valve_miles_interval']))."',
				valve_miles_last='".sql_friendly(money_strip($_POST['valve_miles_last']))."',
				valve_miles_last_date='".(trim($_POST['valve_miles_last_date'])!=""  ? "".date("Y-m-d",strtotime($_POST['valve_miles_last_date']))." 00:00:00" :"0000-00-00 00:00:00")."',
				
				use_pm_oil_report_drain='".(isset($_POST['use_pm_oil_report_drain']) ? 1 : 0)."',
				drain_miles_interval='".sql_friendly(money_strip($_POST['drain_miles_interval']))."',
				drain_miles_last='".sql_friendly(money_strip($_POST['drain_miles_last']))."',
				drain_miles_last_date='".(trim($_POST['drain_miles_last_date'])!=""  ? "".date("Y-m-d",strtotime($_POST['drain_miles_last_date']))." 00:00:00" :"0000-00-00 00:00:00")."',
				...
				has_apu = '".(isset($_POST['has_apu']) ? '1' : '0')."',
				...
				geotab_device_id='".sql_friendly(trim($_POST['geotab_device_id']))."',
				geotab_device_num = '".sql_friendly((int) $_POST['geotab_device_num'])."',
				geotab_device_serial='".sql_friendly(trim($_POST['geotab_device_serial']))."',
     			geotab_device_type='".sql_friendly(trim($_POST['geotab_device_type']))."',
     			geotab_device_plan='".sql_friendly(trim($_POST['geotab_device_plan']))."',
     			
     			geotab_odometer_update_mode ='".sql_friendly($_POST['geotab_odometer_update_mode'])."', 
				...
				truck_max_cruise = '".sql_friendly((int) $_POST['truck_max_cruise'])."',
				truck_max_speed = '".sql_friendly((int) $_POST['truck_max_speed'])."',
				
				image_rotation='".sql_friendly($_POST['image_rotation'])."',
				truck_photo='".sql_friendly($_POST['truck_photo'])."'			
...
?>
<?
$usetitle = "Power Master";
$use_title = "Power Master";

$col_width=15;
$col_height=15;
$col_arr[0]="FFFFFF";                $col_name[0]="White";
$col_arr[1]="000000";                $col_name[1]="Black";
$col_arr[2]="800000";                $col_name[2]="Maroon";
$col_arr[3]="FF0000";                $col_name[3]="Red";
$col_arr[4]="A52A2A";                $col_name[4]="Brown";
$col_arr[5]="FFA500";                $col_name[5]="Orange";
$col_arr[6]="FFFF00";                $col_name[6]="Yellow";
$col_arr[7]="FFD700";                $col_name[7]="Gold";
$col_arr[8]="D2B48C";                $col_name[8]="Tan";
$col_arr[9]="808000";                $col_name[9]="Olive";
$col_arr[10]="7CFC00";               $col_name[10]="Lawn Green";
$col_arr[11]="006400";               $col_name[11]="Dark Green";
$col_arr[12]="008000";               $col_name[12]="Green";
$col_arr[13]="00FF00";               $col_name[13]="Lime";
$col_arr[14]="008080";               $col_name[14]="Teal";
$col_arr[15]="00FFFF";               $col_name[15]="Aqua";
$col_arr[16]="40E0D0";               $col_name[16]="Turquoise";
$col_arr[17]="4682B4";               $col_name[17]="Steel Blue";
$col_arr[18]="1E90FF";               $col_name[18]="Dodger Blue";
$col_arr[19]="87CEFA";               $col_name[19]="Sky Blue";
$col_arr[20]="0000FF";               $col_name[20]="Blue";
$col_arr[21]="4169E1";               $col_name[21]="Royal Blue";
$col_arr[22]="800080";               $col_name[22]="Purple";
$col_arr[23]="EE82EE";               $col_name[23]="Violet";
$col_arr[24]="F4A460";               $col_name[24]="Sandy Brown";
$col_arr[25]="F0E68C";               $col_name[25]="Khaki";
$col_arr[26]="708090";               $col_name[26]="Slate Grey";
$col_arr[27]="808080";               $col_name[27]="Grey";
$col_arr[28]="D3D3D3";               $col_name[28]="Light Grey";
$col_arr[29]="C0C0C0";               $col_name[29]="Silver";
//$col_arr[30]="";               $col_name[30]="";

$old_truck_value="0.00";
?>
<? include('header.php') ?>
<?
	...
	function show_truck($truck_id,$last_pass="",$replacer=0) 
	{
		...
		$sold_it="";        
		if($row['truck_is_sold'] > 0)			    $sold_it="&nbsp;&nbsp; <span class='alert'><b>SOLD!</b></span>";
		...
		if($is_del1==0)
		{
     		...
     		<tr class="<?=$use_class?>">
     			<td>
     				...
     				&nbsp;&nbsp; <?= $warning ?><?=$mrr_insur ?><?=$sold_it ?><?=$on_hold ?><?=$in_shop ?><?=$no_plate ?>
     			</td>
                ...
                <td>
                     ...
                     {
                          echo "
                                <div style='display:inline-block; border:1px solid black; width:" . $col_width . "px; height:" . $col_height . "px; background:#" . $row['truck_color'] . ";'>
                                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                </div>  
                             ";
                     }
                ...
                <td><?=$row['truck_year']?></td>			
     			...
     			<td align='right'>
     				...
     				$pm_odom_date=date("Y-m-d",strtotime($row['geotab_last_odometer_date']));
     				$pm_odom=$row['geotab_last_odometer_reading'];
     				...
     				echo "<span title='Last PM Inspection was done ".$pm_inspect.".".$pm_title_adder." --Last Odometer Reading ".$pm_odom_date."' style='color:#".$pm_color.";'>".number_format($pm_odom,2)."</span>";
     				?>
     			</td>	
     			<td align='right'>
     				<?
     			...
     			<td><?=($row['peoplenet_tracking'] > 0 ? 'Geo' : '')?></td>
     			<?				
     		...
     		{	
     			...
     			$sold_it="";        
     			if($row2['truck_is_sold'] > 0)			$sold_it="&nbsp;&nbsp; SOLD!";
     			...
     			//{
               		...
               		<tr class="<?=$use_class?>">
               			<td>
               				...
               				&nbsp;&nbsp; <?= $warning ?><?=$mrr_insur ?><?=$on_hold ?><?=$sold_it ?><?=$in_shop ?><?=$no_plate ?>
               			</td>
                        ...
                        <td>
                             ...
                             {
                                echo "
                                 <div style='display:inline-block; border:1px solid black; width:" . $col_width . "px; height:" . $col_height . "px; background:#" . $row2['truck_color'] . ";'>
                                     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                 </div>  
                                ";
                             }
          				...
          				<td align='right'>
               				...
               				$pm_odom_date=date("Y-m-d",strtotime($row2['geotab_last_odometer_date']));
     						$pm_odom=$row2['geotab_last_odometer_reading'];
               			...
               			<td><?=($row2['peoplenet_tracking'] > 0 ? 'Geo' : '')?></td>
...
<tr>
	<td valign='top'>
		...
		<tr>
			<td colspan='20'><font class='standard18'><b>Power Master (a.k.a. Truck Master or Admin Trucks)</b></font></td>
		</tr>
		<tr>
			<td colspan='20'>
				<a href="<?=$SCRIPT_NAME?>?id=0">Add New Truck</a>
                <? if($_SESSION['user_id']==23 || $_SESSION['user_id']==15 || $_SESSION['user_id']==18) { ?>
                    &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 
                    &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
		...
		<tr>
			<td colspan='20'>
				...
				<span class='section_heading'><?=get_active_truck_count()?> Active Truck(s) &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <?=get_active_truck_count_exclude()?> Excluded Truck(s)</span>
			</td>
		</tr>
		<tr>
			<td colspan='15'>
				...
				<select name='sort_items_by' onChange='submit();'>
					<option value='name_truck' <?= ($_POST['sort_items_by']=="name_truck" ? " selected" : "")?>>Truck Name</option>
					...
					<option value='license_plate_no' <?= ($_POST['sort_items_by']=="license_plate_no" ? " selected" : "")?>>License Plate</option>
				</select>	
		...
		<tr>
			...
			<td><b>&nbsp;</b></td>
			<td><span title='GeoTab Tracking enabled if PN displays for each Truck...'><b>PN</b></span></td>
			<td><b>Unit</b></td>
			<td><b></b></td>
		</tr>
		<? 
		...
		?>
		<tr>
			<td colspan='20'><center><input type='submit' value='Update Truck Costs' class='mrr_button_access'></center></td>
		</tr>
		</table>
		<?
		echo "<br><center><span style='color:#0000CC; cursor:pointer;' onClick='mrr_run_maint_request_report();'><b>Run Truck Oil/Valve/Drain/PM/FED Report</b></span><center><br>";
		//<a href=\"admin_trucks.php?pmi_fed=1\" target='_blank'>Run Truck Oil/PM/FED Report</a>
		?>
		</form>
	</td>
	<td valign='top' style='width:350px'>
		<? if(isset($_GET['id'])) { 
			...
			<tr>
				<td>
					<form action="<?=$SCRIPT_NAME?>?id=<?=$_GET['id']?>" enctype="multipart/form-data" method="post" name='my_form'>
					...
					<?
						...
						$_POST['linedate_own_op_ins_exp']=date("m/d/Y",strtotime($row_edit['linedate_own_op_ins_exp']));
						if($row_edit['linedate_own_op_ins']=="0000-00-00 00:00:00")			$_POST['linedate_own_op_ins']="";						
						if($row_edit['linedate_own_op_ins_exp']=="0000-00-00 00:00:00")		$_POST['linedate_own_op_ins_exp']="";
						
						$_POST['linedate_own_op_acc_ins']=date("m/d/Y",strtotime($row_edit['linedate_own_op_acc_ins']));
						$_POST['linedate_own_op_acc_ins_exp']=date("m/d/Y",strtotime($row_edit['linedate_own_op_acc_ins_exp']));
						if($row_edit['linedate_own_op_acc_ins']=="0000-00-00 00:00:00")		$_POST['linedate_own_op_acc_ins']="";
						if($row_edit['linedate_own_op_acc_ins_exp']=="0000-00-00 00:00:00")	$_POST['linedate_own_op_acc_ins_exp']="";
						
						$_POST['rfid_tag_number']=trim($row_edit['rfid_tag_number']);
						
						$_POST['max_speed_cruise'] = $row_edit['max_speed_cruise'];
                        $_POST['max_speed_pedal'] = $row_edit['max_speed_pedal'];
						...
						$_POST['valve_miles_interval']=(int) $row_edit['valve_miles_interval'];						
						$_POST['valve_miles_last']=(int) $row_edit['valve_miles_last'];
						$_POST['valve_miles_last_date']="";
						if($row_edit['valve_miles_last_date']!="0000-00-00 00:00:00")		$_POST['valve_miles_last_date']=date("m/d/Y",strtotime($row_edit['valve_miles_last_date']));
                    
                        $_POST['drain_miles_interval']=(int) $row_edit['drain_miles_interval'];
                        $_POST['drain_miles_last']=(int) $row_edit['drain_miles_last'];
                        $_POST['drain_miles_last_date']="";
                        if($row_edit['drain_miles_last_date']!="0000-00-00 00:00:00")		$_POST['drain_miles_last_date']=date("m/d/Y",strtotime($row_edit['drain_miles_last_date']));
					...
					<tr>
						<td><b>Truck</b> <?= show_help('admin_trucks.php','Truck Name') ?> &nbsp; &nbsp; &nbsp; &nbsp; (Should be Numeric for COAs)</td>
						<td colspan='2'>
					...
					<tr>
          				...
          				<td colspan='2'>
          					...
          					Starts 
          					<input name="linedate_own_op_ins" id='linedate_own_op_ins' value="<? if(strtotime($row_edit['linedate_own_op_ins']) != 0) echo date("m/d/Y", strtotime($row_edit['linedate_own_op_ins'])) ?>" style='width:80px;'> 
          					Ends
          					<input name="linedate_own_op_ins_exp" id='linedate_own_op_ins_exp' value="<? if(strtotime($row_edit['linedate_own_op_ins_exp']) != 0) echo date("m/d/Y", strtotime($row_edit['linedate_own_op_ins_exp'])) ?>" style='width:80px;'> 
          					(mm/dd/yyyy)
          			...
          			<tr>
          				...
          				<td colspan='2'>
          					...
          					Starts 
          					<input name="linedate_own_op_acc_ins" id='linedate_own_op_acc_ins' value="<? if(strtotime($row_edit['linedate_own_op_acc_ins']) != 0) echo date("m/d/Y", strtotime($row_edit['linedate_own_op_acc_ins'])) ?>" style='width:80px;'> 
          					Ends
          					<input name="linedate_own_op_acc_ins_exp" id='linedate_own_op_acc_ins_exp' value="<? if(strtotime($row_edit['linedate_own_op_acc_ins_exp']) != 0) echo date("m/d/Y", strtotime($row_edit['linedate_own_op_acc_ins_exp'])) ?>" style='width:80px;'> 
          					(mm/dd/yyyy)
					...
					<tr>
						<td><label for='peoplenet_tracking'><b>Geotab Tracking</b></label> <?= show_help('admin_trucks.php','PeopleNet Tracking') ?></td>
					...
					<tr>
						<td><label for='has_inverter'><b>Inverter</b></label> <?= show_help('admin_trucks.php','has_inverter') ?></td>
						<td colspan='2'><input type='checkbox' name='has_inverter' id='has_inverter' <? if($row_edit['has_inverter']) echo 'checked'?>></td>
					</tr>
					...
					<tr>
                        ...
                        <? /*
					<tr>
						<td><b>POdometer Offset</b> <?= show_help('admin_trucks.php','pn_odometer_offset') ?></td>
					...
					<tr>
						...
						<td colspan='2'>
                            ...
                            <input type='hidden' name='pn_odometer_offset' id='pn_odometer_offset' value="<?=$row_edit['pn_odometer_offset']?>">
						</td>
					</tr>
					<tr>
					...
					<tr class='mrr_shader mrr_hide_sector'>
						...
						<td valign='top' colspan='2'>
                            ...
                            <b>Max Speeds (MPH): <?= show_help('admin_trucks.php','Max Truck Speeds') ?></b> 
                            
                            <b>Cruise</b> <input id="max_speed_cruise" name="max_speed_cruise" value="<?=trim($row_edit['max_speed_cruise'])?>" size="2" style="text-align:right;">

                            <b>Foot/Pedal</b> <input id="max_speed_pedal" name="max_speed_pedal" value="<?=trim($row_edit['max_speed_pedal'])?>" size="2" style="text-align:right;">
                            <br>
                    ...
                    <tr>
                        <td><label for='truck_is_sold'><b>Truck is SOLD</b></label> <?= show_help('admin_trucks.php','truck_is_sold') ?></td>
                        <td colspan='2'>
                            <input type='checkbox' name='truck_is_sold' id='truck_is_sold' <? if($row_edit['truck_is_sold']) echo 'checked'?>>
                        </td>
                    </tr>
					<tr>
					...
					<tr>
						...
						<? if($use_admin_level > 90) { ?>
							...
							{
								if($old_truck_value=="0.00")		$old_truck_value=$row_history['equipment_value'];
								...
								echo "
									<tr>
										...
										<td align='right'><input name='equipment_value_$row_history[id]' value=\"$".money_format('',$row_history['equipment_value'])."\" style='width:80px;text-align:right' class='equip_history_value'></td>
							...
							<tr>
								<td colspan='5'>
									...
									&nbsp; &nbsp; &nbsp; &nbsp;
									<input type='button' value='Update Value Only' onClick="mrr_update_value_only();" class='mrr_button_access'>
									</center>
								</td>
							</tr>							
							<tr>
                    ...
                    <tr>
                        <td><b>Max Speed</b> <?= show_help('admin_trucks.php','truck_max_speed') ?></td>
                        <td colspan='2'><input name="truck_max_speed" value="<?=$row_edit['truck_max_speed']?>"> MPH</td>
                    </tr>
                    <tr>
                        <td><b>Max Cruise</b> <?= show_help('admin_trucks.php','truck_max_cruise') ?></td>
                        <td colspan='2'><input name="truck_max_cruise" value="<?=$row_edit['truck_max_cruise']?>"> MPH</td>
                    </tr>
					...
					<tr>
						...
						<td colspan='2'><input name="vin" id="vin" value="<?=$row_edit['vin']?>" onBlur="mrr_validate_duped_info();"></td>
					</tr>
					<tr>
                    ...
                    <tr>
                        <td><b>Truck Color</b> <?= show_help('admin_trucks.php','truck_color') ?></td>
                        <td colspan='2'>
                             <select name="truck_color" id="truck_color">
                                 <option value=""<?=($row_edit['truck_color']=="" ? " selected" : "")?>>N/A</option>
                                  <?php
                                  for($i=0; $i<count($col_arr); $i++)
                                  {
                                      echo "
                                            <option value='".$col_arr[$i]."'".($row_edit['truck_color']==$col_arr[$i] ? " selected" : "")." style=' background:#".$col_arr[$i].";'>                                                 
                                                
                                                ".$col_name[$i]."                                                
                                            </option>
                                      ";        //<img src='mrr_jpg.png' alt='' style='border:1px solid black; width:".$col_width."px; height:".$col_height."px;'>
                                  }
                                  ?>
                             </select>
                             Preview
                             <?
                             if($row_edit['truck_color']!="") 
                             {
                                  echo "
                                <div style='display:inline-block; border:1px solid black; width:" . $col_width . "px; height:" . $col_height . "px; background:#" . $row_edit['truck_color'] . ";'>
                                    &nbsp;
                                </div>  
                                ";
                             }
                             ...
                             {    
                                echo "<i>- No color selected.</i>";
                             }
                             ?>                             
                        </td>
                    </tr>    
					<tr>
						<td><b>GeoTab Device ID</b> <?= show_help('admin_trucks.php','GeoTab Device ID') ?></td>
						<td colspan='2'><input name="geotab_device_id" id="geotab_device_id" value="<?= trim($row_edit['geotab_device_id'])?>" onBlur="mrr_validate_duped_info();"></td>
					</tr>
					<tr>
						<td><b>GeoTab Device Number</b> <?= show_help('admin_trucks.php','GeoTab Device Num') ?></td>
						<td colspan='2'><input name="geotab_device_num" value="<?=(int) $row_edit['geotab_device_num']?>"></td>
					</tr>
					<tr>
						<td><b>GeoTab Device Serial No.</b> <?= show_help('admin_trucks.php','GeoTab Device Serial No.') ?></td>
						<td colspan='2'><input name="geotab_device_serial" id="geotab_device_serial" value="<?=trim($row_edit['geotab_device_serial'])?>" onBlur="mrr_validate_duped_info();"></td>
					</tr>
					<tr>
						<td><b>GeoTab Device GoType</b> <?= show_help('admin_trucks.php','GeoTab Device Go Type') ?></td>
						<td colspan='2'><input name="geotab_device_type" value="<?=trim($row_edit['geotab_device_type'])?>"></td>
					</tr>
					<tr>
						<td><b>GeoTab Device GoPlan</b> <?= show_help('admin_trucks.php','GeoTab Device Go Plan') ?></td>
						<td colspan='2'><input name="geotab_device_plan" value="<?=trim($row_edit['geotab_device_plan'])?>"></td>
					</tr>
					<tr>
						<td valign='top'><b>GeoTab Last Odometer</b> <?= show_help('admin_trucks.php','GeoTab Last Odometer') ?></td>
						<td valign='top' colspan='2'>
							<?= trim($row_edit['geotab_last_odometer_reading'])?>
							<i>as of <?= ($row_edit['geotab_last_odometer_date']!='0000-00-00 00:00:00' ? "".date("m/d/Y H:i:s",strtotime($row_edit['geotab_last_odometer_date']))."" : "<b>N/A</b>") ?></i>
							<?
								$use_device_id=trim($row_edit['geotab_device_id']);
								if($use_device_id!="")
								{
									?>
									&nbsp;&nbsp;&nbsp; Update: 
									<a href='geotab.php?diagnotics=1&device_id=<?=$use_device_id ?>' target='_blank' title='ADJUSTED ODOMETER--Click this link to grab the last odometer reading form GeoTab... Use if automatic process is not working for his truck.'>
										D1
									</a>
									&nbsp;&nbsp;&nbsp;
									<a href='geotab.php?diagnotics=2&device_id=<?=$use_device_id ?>' target='_blank' title='REGULAR ODOMETER--Click this link to grab the last odometer reading form GeoTab... Use if automatic process is not working for his truck.'>
										D2
									</a>
									<br>
									<br>Last <a href='geotab_cronjob.php?feed_type=101&device_id=<?=$use_device_id ?>&last_time=<?=strtotime($row_edit['geotab_last_location_date']) ?>' target='_blank'>Location Record</a> from GeoTab: 
										<?=date("m/d/Y H:i:s",strtotime($row_edit['geotab_last_location_date'])) ?>.
									<br>Bypass/Skip truck for 
										<span style='color:blue; cursor:pointer;' onClick='mrr_bypass_geotab_location_date(2);'>2-hrs</span>,
										<span style='color:blue; cursor:pointer;' onClick='mrr_bypass_geotab_location_date(6);'>6-hrs</span>,
										<span style='color:blue; cursor:pointer;' onClick='mrr_bypass_geotab_location_date(24);'>Today</span>,
										<span style='color:blue; cursor:pointer;' onClick='mrr_bypass_geotab_location_date(168);'>Week</span>,
										<span style='color:blue; cursor:pointer;' onClick='mrr_bypass_geotab_location_date(672);'>Month</span>,
										<span style='color:blue; cursor:pointer;' onClick='mrr_bypass_geotab_location_date(8064);'>Year</span>,
										<br> or
										<span style='color:blue; cursor:pointer;' onClick='mrr_bypass_geotab_location_date(0);'>Restore (turn off bypass/skip) Auto-Run.</span>
									<?									
								}
							?>							
						</td>
                    ...
                    <tr>
                        <td valign='top'><b>Current Location:</b></td>
                        <td valign='top' colspan='2'>
                             <?=trim($row_edit['geotab_current_location']) ?>
                        </td>
                    </tr>
					<tr>
						<td><b>Odometer Update Mode</b> <?= show_help('admin_trucks.php','GeoTab Last Odometer') ?></td>
						<td colspan='2'>
							<select name='geotab_odometer_update_mode'>
								<option value='1'<?= ($row_edit['geotab_odometer_update_mode'] ==1 ? " selected" : "") ?>>D1 - Adjusted Mode</option>
								<option value='0'<?= ($row_edit['geotab_odometer_update_mode'] !=1 ? " selected" : "") ?>>D2 - Standard Mode</option>
							</select>		
						</td>
					</tr>	
					<tr>
						<td><b>Has APU</b> <?= show_help('admin_trucks.php','has_apu') ?></td>
						<td colspan='2'><input type='checkbox' name='has_apu' id='has_apu' <? if($row_edit['has_apu']) echo 'checked'?>></td>
					</tr>						
					<tr>
						<td><b>ELD and Camera Unit</b> <?= show_help('admin_trucks.php','PN Unit Type') ?></td>
					...
					<tr>
                        ...
                        */ ?>
					<tr>
						<td><b>ELD and Camera Unit Value</b> <?= show_help('admin_trucks.php','PN Unit Type') ?></td>
						<td colspan='2'>
                            <input name="apu_value" value="<?=$row_edit['apu_value']?>" size='10' style='text-align:right;'>

                            <input type="hidden" name="apu_serial" value="<?=$row_edit['apu_serial']?>">
                            <input type="hidden" name="dvr_serial" value="<?=$row_edit['dvr_serial']?>">
                        </td>
					</tr>
					<tr>
						<td nowrap><b>License Plate #</b> <?= show_help('admin_trucks.php','License Plate') ?></td>
						<td colspan='2'><input name="license_plate_no" id="license_plate_no" value="<?=$row_edit['license_plate_no']?>" onBlur="mrr_validate_duped_info();"></td>
					</tr>
                    <tr>
                        <td nowrap><b>Cab Card Exp.</b> <?= show_help('admin_trucks.php','linedate_cab_card') ?></td>
                        <td colspan='2'><input name="linedate_cab_card" id="linedate_cab_card" value='<?=($row_edit['linedate_cab_card']!='0000-00-00 00:00:00' ? "".date("m/d/Y",strtotime($row_edit['linedate_cab_card']))."" : "") ?>' class='datepicker' placeholder='mm/dd/YYYY'></td>
                    </tr>
                    <tr>
                        <td nowrap><b>RFID Tag #</b> <?= show_help('admin_trucks.php','RFID Tag No') ?></td>
                        <td colspan='2'><input name="rfid_tag_number" value="<?=$row_edit['rfid_tag_number']?>"></td>
                    </tr>    
					<tr>
						<td nowrap><b>Prepass</b> <?= show_help('admin_trucks.php','Prepass') ?></td>
						<td colspan='2'><input name="prepass" id="prepass" value="<?=$row_edit['prepass']?>" onBlur="mrr_validate_duped_info();"></td>
					</tr>

                    <tr>
                        <td nowrap><b>Tablet IMEI</b> <?= show_help('admin_trucks.php','Tablet IMEI') ?></td>
                        <td colspan='2'><input name="tablet_imei" id="tablet_imei" value="<?=$row_edit['tablet_imei']?>" onBlur="mrr_validate_duped_info();"></td>
                    </tr>
                    <tr>
                        <td nowrap><b>Zenduit DRID</b> <?= show_help('admin_trucks.php','Zenduit DRID') ?></td>
                        <td colspan='2'><input name="zenduit_drid" id="zenduit_drid" value="<?=$row_edit['zenduit_drid']?>" onBlur="mrr_validate_duped_info();"></td>
                    </tr>    
					<tr>
						<td nowrap><b>IFTA Decal Exp.</b> <?= show_help('admin_trucks.php','linedate_ifta') ?></td>
						<td colspan='2'><input name="linedate_ifta" id="linedate_ifta" style='width:80px' value='<?=($row_edit['linedate_ifta']!='0000-00-00 00:00:00' ? "".date("m/d/Y",strtotime($row_edit['linedate_ifta']))."" : "") ?>' class='datepicker' placeholder='mm/dd/YYYY'></td>
					</tr>
					<tr>
						<td nowrap><b>IFTA Decal No.</b> <?= show_help('admin_trucks.php','ifta_number') ?></td>
						<td colspan='2'><input name="ifta_number" value="<?=$row_edit['ifta_number']?>"></td>
					</tr>
                    <tr>
                        <td valign='top'>
                            <b>Truck Photo</b> <?= show_help('admin_trucks.php','truck_photo') ?>
                            <br> (.gif, .jpg, or .png)
                        </td>
                        <td colspan='2' valign='top'>
                            <input name="truck_photo"  id="truck_photo" value="<?=$row_edit['truck_photo']?>">
                            <br>
                            <input type="file" name='mrr_import' id='mrr_import' style='width:200px;'>
                             <?
                             $mrr_truck_image=trim($row_edit['truck_photo']);
                    ...
                    <tr>
                        <td valign='top'>
                            <b>Rotation</b> <?= show_help('admin_trucks.php','image_rotation') ?>
                        </td>
                        <td colspan='2' valign='top'>
                    ...
                    <tr>
                        <td colspan='3' valign='top'>
                             <?
                             if(trim($mrr_truck_image)!="")
                             {
                                  $sized = getimagesize("trucks/".trim($mrr_truck_image)."");
                                  ...
                                  echo "
                                    <br>
                                    <br>
                                    <a href='/trucks/".trim($mrr_truck_image)."' target='_blank'>
                                        <img src='/trucks/".trim($mrr_truck_image)."' border='1' width='".$iwide."' alt='".trim($mrr_truck_image)." not found...".$iwide." x ".$ihigh."'".
                    ...
                    $drain_over_due=(int) trim(str_replace(",","",$defaultsarray['truck_drain_separator_due']));
                    $drain_almost_due=(int) trim(str_replace(",","",$defaultsarray['truck_drain_separator_soon']));
                    $drain_due_sooner_miles=$drain_over_due - $drain_almost_due;
					
					$pm_odom_date=date("m/d/Y H:i",strtotime($row_edit['geotab_last_odometer_date']));
     				$pm_odom=$row_edit['geotab_last_odometer_reading'];
					?>
					<div class='clear'></div>
					<br>
					<div id='pm_inspection_section' style='margin-bottom:10px;'>
						<table class='admin_menu1' style='width:100%;margin-bottom:10px'>
     					...
     					<tr>
     						...
     						<td align='right'>&nbsp; <input type="hidden" name="pm_miles_update_oil" id="pm_miles_update_oil" value="0"></td>
     					</tr>
     					<tr>
     					...
     					<tr>
     						...
     						<td colspan='2'><?=number_format($pm_odom,2) ?> <i>As of <?=$pm_odom_date?></i></td>
     					</tr>
     					<?
     					$pm_odom_due=0;
     					$pm_title_adder="";
     					...
     					<tr>
     						<td colspan='2' align='right'><b>Use UPDATE section above to set or use this for current Odometer - > </b></td>
     						<td align='right'><input type='button' value="Oil Changed" class='mrr_button_access' onClick='mrr_oil_changed(<?=(int) $pm_odom ?>);'></td>
     					</tr>
     					
     					<tr>
     						<td colspan='4' class='border_bottom'><div class='section_heading'>Valve Adjustment (Overhead)</div></td>
     					</tr>
     					<tr>
     						<td><label for='use_pm_oil_report_valve'><b>Use on Oil/PM/FED Report:</b></label> <?= show_help('admin_trucks.php','use_pm_oil_report_valve') ?></td>
     						<td><input type='checkbox' name='use_pm_oil_report_valve' id='use_pm_oil_report_valve' <? if($row_edit['use_pm_oil_report_valve']) echo 'checked'?>></td>
     						<td align='right'>&nbsp;<input type="hidden" name="pm_miles_update_avi" id="pm_miles_update_avi" value="0"></td>
     					</tr>
     					<tr>
     						<td><b>Valve Mileage Interval:</b> <?= show_help('admin_trucks.php','Valve Mileage Interval') ?></td>
     						<td><input name='valve_miles_interval' id='valve_miles_interval' style='width:80px; text-align:right;' value='<?=$_POST['valve_miles_interval'] ?>'></td>
     						<td align='right'><span style='color:#0000cc; cursor:pointer;' onClick='mrr_fill_valve_interval(<?=$over_due ?>)'>< - - <?=$over_due ?></span></td>
     					</tr>
     					<tr>
     						<td><b>Odometer when Valve Last Adjusted:</b> <?= show_help('admin_trucks.php','Odometer when Valve Last Adjusted') ?></td>
     						<td><input name='valve_miles_last' id='valve_miles_last' style='width:80px; text-align:right;' value='<?=$_POST['valve_miles_last'] ?>'></td>
     						<td align='right'> Warn at <?=$almost_due ?></td>
     					</tr>
     					<tr>
     						<td><b>Last Date Valve Adjusted:</b> <?= show_help('admin_trucks.php','Last Date Valve Adjusted') ?></td>
     						<td><input name='valve_miles_last_date' id='valve_miles_last_date' style='width:80px' value='<?=$_POST['valve_miles_last_date'] ?>' class='datepicker' placeholder='mm/dd/YYYY'></td>
     						<td align='right'>&nbsp;</td>
     					</tr>
     					<tr>
     						<td><b>Current Odometer Reading:</b> <?= show_help('admin_trucks.php','PM Odometer Reading') ?></td>
     					...
     					if($_POST['valve_miles_last_date']!="0000-00-00 00:00:00")		$valve_inspect="".date("m/d/Y",strtotime($_POST['valve_miles_last_date']))."";
     					
     					
     					$valve_odom_due=((int) $_POST['valve_miles_last'] + (int) $_POST['valve_miles_interval']);
     					
     					if($valve_odom_due <= $pm_odom)
     					{
     						$valve_color="CC0000";
     						$valve_title_adder=" Was Due at ".$valve_odom_due." Miles and is ".($valve_odom_due - $pm_odom)." overdue.";
     					}
     					elseif(($valve_odom_due - $pm_odom) <=($over_due - $almost_due ) )
     					{										//$due_sooner_miles
     						$valve_color="FFCC00";
     						$valve_title_adder=" Is coming Due at ".$valve_odom_due." Miles -- ".($valve_odom_due - $pm_odom)." left.";
     					}
     					else
     					{
     						$valve_color="00CC00";
     						$valve_title_adder=" Will be Due at ".$valve_odom_due." Miles.";	
     					}
     					
     					?>
     					<tr>
     						<td><b>Next Odometer Reading Due:</b> <?= show_help('admin_trucks.php','PM Valve Adjustment Due') ?></td>
     						<td><span title='Last Valve Adjustment was done <?=$valve_inspect ?>. <?=$valve_title_adder ?>' style='color:#<?=$valve_color ?>;'><?=$valve_odom_due ?></span></td>
     						<td align='right'>&nbsp;</td>
     					</tr> 
     					<tr>
     						<td colspan='2' align='right'><b>Use UPDATE section above to set or use this for current Odometer - > </b></td>
     						<td align='right'><input type='button' value="Valve Adjusted" class='mrr_button_access' onClick='mrr_valve_changed(<?=(int) $pm_odom ?>);'></td>
     					</tr>
                        <tr>
                            <td colspan='4' class='border_bottom'><div class='section_heading'>Drain Water Separator</div></td>
                        </tr>
                        <tr>
                            <td><label for='use_pm_oil_report_drain'><b>Use on Oil/PM/FED Report:</b></label> <?= show_help('admin_trucks.php','use_pm_oil_report_drain') ?></td>
                            <td><input type='checkbox' name='use_pm_oil_report_drain' id='use_pm_oil_report_drain' <? if($row_edit['use_pm_oil_report_drain']) echo 'checked'?>></td>
                            <td align='right'>&nbsp;<input type="hidden" name="pm_miles_update_drain" id="pm_miles_update_drain" value="0"></td>
                        </tr>
                        <tr>
                            <td><b>Drain Separator Mileage Interval:</b> <?= show_help('admin_trucks.php','Drain Water Separator Mileage Interval') ?></td>
                            <td><input name='drain_miles_interval' id='drain_miles_interval' style='width:80px; text-align:right;' value='<?=$_POST['drain_miles_interval'] ?>'></td>
                            <td align='right'><span style='color:#0000cc; cursor:pointer;' onClick='mrr_fill_drain_interval(<?=$drain_over_due ?>)'>< - - <?=$drain_over_due ?></span></td>
                        </tr>
                        <tr>
                            <td><b>Odometer when Drain Last Separated:</b> <?= show_help('admin_trucks.php','Odometer when Drain Last Separated') ?></td>
                            <td><input name='drain_miles_last' id='drain_miles_last' style='width:80px; text-align:right;' value='<?=$_POST['drain_miles_last'] ?>'></td>
                            <td align='right'> Warn at <?=$drain_almost_due ?></td>
                        </tr>
                        <tr>
                            <td><b>Last Date Drain Separated:</b> <?= show_help('admin_trucks.php','Last Date Drain Separated') ?></td>
                            <td><input name='drain_miles_last_date' id='drain_miles_last_date' style='width:80px' value='<?=$_POST['drain_miles_last_date'] ?>' class='datepicker' placeholder='mm/dd/YYYY'></td>
                            <td align='right'>&nbsp;</td>
                        </tr>
                        <tr>
                            <td><b>Current Odometer Reading:</b> <?= show_help('admin_trucks.php','PM Odometer Reading') ?></td>
                         ...
                         if($_POST['drain_miles_last_date']!="0000-00-00 00:00:00")		$drain_inspect="".date("m/d/Y",strtotime($_POST['drain_miles_last_date']))."";
                         
                         
                         $drain_odom_due=((int) $_POST['drain_miles_last'] + (int) $_POST['drain_miles_interval']);
                         
                         if($drain_odom_due <= $pm_odom)
                         {
                              $drain_color="CC0000";
                              $drain_title_adder=" Was Due at ".$drain_odom_due." Miles and is ".($drain_odom_due - $pm_odom)." overdue.";
                         }
                         elseif(($drain_odom_due - $pm_odom) <=($drain_over_due - $drain_almost_due ) )
                         {										//$due_sooner_miles
                              $drain_color="FFCC00";
                              $drain_title_adder=" Is coming Due at ".$drain_odom_due." Miles -- ".($drain_odom_due - $pm_odom)." left.";
                         }
                         else
                         {
                              $drain_color="00CC00";
                              $drain_title_adder=" Will be Due at ".$drain_odom_due." Miles.";
                         }
                         
                         ?>
                        <tr>
                            <td><b>Next Odometer Reading Due:</b> <?= show_help('admin_trucks.php','PM Drain Separated Due') ?></td>
                            <td><span title='Last Drain Separated done <?=$drain_inspect ?>. <?=$drain_title_adder ?>' style='color:#<?=$valve_color ?>;'><?=$drain_odom_due ?></span></td>
                            <td align='right'>&nbsp;</td>
                        </tr>
                        <tr>
                            <td colspan='2' align='right'><b>Use UPDATE section above to set or use this for current Odometer - > </b></td>
                            <td align='right'><input type='button' value="Drain Separated" class='mrr_button_access' onClick='mrr_drain_changed(<?=(int) $pm_odom ?>);'></td>
                        </tr>
     					</table>	
					</div>										
					</form>
                            ...
                            <tr>
                             ...
                             $sql_tasks = "
                                ...
                                where internal_tasks.deleted <= 0 and internal_tasks_checked.deleted<=0
                                    ...
                                    and internal_tasks.task_type = 1
					...
					if($use_admin_level > 95)
					{
						...
						<table class='admin_menu2' style='width:100%;margin-bottom:10px'>
						<tr>
							<td colspan='5' class='border_bottom'><div class='section_heading'>Value History</div></td>
						</tr>
						<tr>
							<td><b>Recorded</b></td>
							<td><b>Acquired</b></td>
...
<script type='text/javascript'>
	...
	function mrr_validate_duped_info()
    {
        var vin = $('#vin').val();
        var plate = $('#license_plate_no').val();
        var device = $('#geotab_device_id').val();
        var serial = $('#geotab_device_serial').val();
        var prepass = $('#prepass').val();
        var tablet = $('#tablet_imei').val();
        var zenduit = $('#zenduit_drid').val();

        $.ajax({
            type: "POST",
            url: "ajax.php?cmd=mrr_show_truck_info_validation",
            data: { "truck_id":mrr_truck_id ,
                "vin": vin,
                "plate": plate,
                "device": device,
                "serial": serial,
            ...
            success: function(xml) {
                var myCntr=parseInt($(xml).find('mrrCntr').text());
                var mytxt=$(xml).find('mrrTab').text();
                if(myCntr > 0)      $.prompt("Warning: Truck information duplicated:  <br>"+ mytxt +"<br>");
            }
        });
    }
	...
	function mrr_fill_drain_interval(mrr_interval)
	{
		var valve=parseInt(mrr_interval);
		$('#drain_miles_interval').val(''+valve+'');
		
	}
	function mrr_drain_changed(cur_miles)
	{
		var cmiles=cur_miles;
		$('#drain_miles_last').val(''+cmiles+'');
        $('#pm_miles_update_drain').val('1');
		document.my_form.submit();
	}

    function mrr_fill_valve_interval(mrr_interval)
    {
        var valve=parseInt(mrr_interval);
        $('#valve_miles_interval').val(''+valve+'');

    }
    function mrr_valve_changed(cur_miles)
    {
        var cmiles=cur_miles;
        $('#valve_miles_last').val(''+cmiles+'');
        $('#pm_miles_update_avi').val('1');
        document.my_form.submit();
    }
	...
	function mrr_bypass_geotab_location_date(hrs)
	{
		//resets the date X hours from now to skip the Auto-Run backup routine to update the current truck...in case one isn't moving.
		
		$.ajax({
		   type: "POST",
		   url: "ajax.php?cmd=mrr_bypass_geotab_location_updater",
		   data: {
		   		"id": mrr_truck_id,
		   		"hrs":hrs
		   	},
		   ...
		   cache:false,
		   success: function(xml) {
		   		$.noticeAdd({text: "Truck GPS Location date set to "+hrs+" hours from now... successfully."});	
		   }
		 });	
	}
...
?>
	function mrr_run_maint_request_report()
	{
		$.ajax({
		   ...
		   success: function (data) {
		   		//WindowObjectReference = window.open("admin_trucks.php?pmi_fed=1", "PMI_FED_report", 'width=850,height=550,location=no,resizable=yes,menubar=no,status=yes,toolbar=no,scrollbars=yes');
				//WindowObjectReference.focus();
				window.location = "admin_trucks.php?pmi_fed=1";
		   }
		 });	
	}
	...
	function mrr_update_value_only()
	{	
		var new_aquired_val=$('.equip_history_value:last').val();	
		var aquired_val="<?=$old_truck_value?>";	
		//mrr_truck_id		
		
		$.ajax({
		   type: "POST",
		   url: "ajax.php?cmd=mrr_update_history_truck_value",
		   data: {"truck_id":mrr_truck_id , "old_value":aquired_val, "new_value":new_aquired_val},
		   dataType: "xml",
		   cache:false,
		   success: function(xml) {
				$.noticeAdd({text: "Truck Value updated successfully."});
		   }
		 });
	}
	...
	$('#linedate_own_op_ins_exp').datepicker();
	$('#linedate_own_op_acc_ins').datepicker();
	$('#linedate_own_op_acc_ins_exp').datepicker();
...
if(isset($_GET['id']) && $_GET['id'] > 0 && 1==2) 
{
	...
	$mrr_tab_sent2=mrr_get_geotab_messages_sent_by_truck($msg_date_from, $msg_date_to, $msg_truck_id, $msg_truck_name, 0, 0, 3,1);
