	...
	<tr>
		...
		<tr>
			...
			<td>&nbsp;
				...
				<input type='checkbox' name='request_truck_breakdown' id='request_truck_breakdown' <? if($request_truck_breakdown) echo 'checked'?>>
				<br>
				<label for='safety_shutdown'><b>Safety Shut Down!</b></label> 
				<input type='checkbox' name='safety_shutdown' id='safety_shutdown' <? if($safety_shutdown) echo 'checked'?>>
			</td>
			<td><b>&nbsp;</b></td>
			<td>&nbsp;</td>
			<td><b>&nbsp;</b></td>
     ...
     function display_maint(type_id, xref_id) 
     {
         ...
         $.ajax({
			...
			success: function(xml) {
				...
				temp_holder+="<td class='list_mode' align='right'><b>Cost</b></td>";
				//temp_holder+="<td><b>Recur</b></td>";
				//temp_holder+="<td><b>Days</b></td>";
				//temp_holder+="<td><b>Miles</b></td>"; 
				
				//temp_holder+="<td><b>Recurs</b></td>";  
				//temp_holder+="<td><b>Tire Size</b></td>";  
				
				temp_holder+="<td><b>Quick Note Or Location</b></td>";      				
				
				temp_holder+="<td class='list_mode'><b>PM Date</b></td>";
				temp_holder+="<td class='list_mode'><b>FED Date</b></td>"; 
				...
				$(xml).find('MaintRequest').each(function() {
					...
					if(tempy==1)	showurg="<span style='color:#CC0000;' title='Urgent Maintenance Request'><b>!!!</b></span>";
					
					tempz=$(this).find('RequestBreakdown').text();
					if(tempz==1)	showurg="<span style='color:#CC0000;' title='Broken Down Unit'><b>BD!</b></span>";
					
					tempx=$(this).find('SafetyShutDown').text();
					if(tempx==1)	
					{
						showurg="<span style='color:#CC0000;' title='Safety Shut Down Unit'><b>SAFETY</b></span>";
						showclass=" style='background-color:#FFFFFF;'";
					}
					
					snoozer="";
					if(parseInt($(this).find('RequestSnooze').text()) > 0)
                    {
                        //RequestSnooze
                        snoozer=" class='mr_snoozing'";
                    }

                    							
					temp_holder+="<tr"+showclass+""+snoozer+">";	
					temp_holder+="<td>"+showurg+"</td>";   				
					temp_holder+="<td>"+$(this).find('RequestLink').text()+"</td>";
					temp_holder+="<td><span id='cur_local_"+$(this).find('RequestID').text()+"_type'>"+$(this).find('RequestType').text()+"<span></td>";
					temp_holder+="<td><span id='cur_local_"+$(this).find('RequestID').text()+"_unit'>"+$(this).find('RequestName').text()+"<span></td>";
					temp_holder+="<td>"+$(this).find('RequestScheduled').text()+"</td>";
					//temp_holder+="<td>"+$(this).find('RequestCompleted').text()+"</td>";
					temp_holder+="<td class='list_mode' align='right'>"+$(this).find('RequestCost').text()+"</td>";     					
					...
					//temp_holder+="<td>"+$(this).find('RequestTires').text()+"</td>";  
					
                    //"+$(this).find('RequestCurLocal').text()+"
					temp_holder+="<td><input type='text' id='cur_local_"+$(this).find('RequestID').text()+"' value=\"\" onBlur='mrr_update_mr_location_alt("+$(this).find('RequestID').text()+");' title='enter the current location of this unit.'></td>"; 
										
					 
					temp_holder+="<td class='list_mode'>"+$(this).find('RequestPM').text()+"</td>";   
					temp_holder+="<td class='list_mode'>"+$(this).find('RequestFED').text()+"</td>";   
					...
					temp_holder+="<tr"+showclass+""+snoozer+">";	
					temp_holder+="<td>&nbsp;</td>";  
					temp_holder+="<td colspan='10'>"+$(this).find('RequestUnitLog').text()+"</td>";  
					temp_holder+="<td>&nbsp;</td>";  
					temp_holder+="</tr>";
				});	
				temp_holder+="</table>";
				...
				if(current_req_id > 0)		$('.list_mode').hide();
