 ...
 jQuery.fn.jqUploader = function(options) {
    return this.each(function(index) {
		...
		TerminateJQUploader = function(containerId,fileName,varname){
			// the following line makes it so the program can't upload subsequent files
			//$this= $('#'+containerId).empty();
			//$this.text('').append('<span style="color:#000000;font-family:arial;font-size:12px;">Upload of <strong>'+filename+'</strong> finished!</span><input name="'+varname+'" type="hidden" id="'+varname+'" value="'+filename+'"/>');
			$this.text('').append('<span style="color:#000000;font-family:arial;font-size:12px;">Upload of <strong>'+fileName+'</strong> finished!</span>');
			$('#'+varname).val(fileName);
			
			// custom routine to launch a javascript function one the file has been uploaded
			if(varname != '') $(after_upload(varname));
