<?
$use_title="Quote -Conard Logistics";
$usetitle ="Quote -Conard Logistics";
?>
<? include('header.php') ?>
<?
	$rate_adjustment=trim($defaultsarray['quote_rate_adjustment']);
	if(!is_numeric($rate_adjustment))		$rate_adjustment=0;
	...
	if(isset($_GET['id'])) {
		...
		while($row_stop = mysqli_fetch_array($data_stops)) {
			...
			if(trim($row_stop['stop_location'])=="" || trim($row_stop['stop_location'])=="00000")
			{
				$_POST['stop_'.$row_stop['stop_order_id']] = $row_stop['stop_city'];	
			}
	...
	if(!isset($_POST['daily_cost'])) $_POST['daily_cost'] = get_daily_cost($_POST['truck_id']);
...
<tr>
	<td valign='top'>
		...
		<tr>
			...
			<td><input type='checkbox' name='load_taken' id='load_taken' <?=($_POST['load_taken'] == '1' ? 'checked' : "")?>></td>
		</tr>
		<tr>
			<td>Quote Name</td>
		...
		<tr>
			<td>Team Drivers?</td>
			<td>
				<label><input type='radio' id='team_drivers' name='team_drivers' value='0'<?= ($_POST['team_drivers'] == 0 ? " checked" : "") ?> onClick='team_driver_change(0);'> No</label>
				 / 
				 <label><input type='radio' id='team_drivers' name='team_drivers' value='1'<?= ($_POST['team_drivers'] > 0 ? " checked" : "") ?> onClick='team_driver_change(1);'> Yes</label>
			</td>
		</tr>
		<tr>
			<td>Date</td>
		...
		<tr>
			<td colspan='2'>
				...
				<? for($i=1;$i<9;$i++) { ?>
					<tr>
						<td>Stop <?=$i?> Zip or City/State</td>
						<td><input name='stop_<?=$i?>' id='stop_<?=$i?>' value="<?=(isset($_POST['stop_'.$i]) ? $_POST['stop_'.$i] : '')?>" class='stop_entry' style='width:250px'></td>
		...
		<tr>
			...
			<td>
				...
				PC*M <input name='pcm_loaded_miles' id='pcm_loaded_miles' value='<?=$_POST['pcm_loaded_miles']?>' style='width:100px;'>
			</td>
		</tr>
		<tr>
		...
		<? if($rate_adjustment > 0 || $rate_adjustment < 0) { ?>
     		<tr>
     			<td valign='top'>Adjustment</td>
     			<td valign='top'>
     				<?  
     				echo "<span style='font-weight:bold; background-color:purple; color:#FFFFFF;'>+ ".number_format(($rate_adjustment * 100),2)."% (<span id='qr_display_val'></span>)</span>";
     				?>
     			</td>
     		</tr>
     		<tr>
     			<td valign='top'>Ask Customer For</td>
     			<td valign='top'>
     				<input name='quote_rate_display' id='quote_rate_display' class='xlocked' style='text-align:right;font-weight:bold; background-color:purple; color:#FFFFFF;'>
     			</td>
     		</tr>
		<? } ?>
		<!---
	...
	<td valign='top' style='text-align:left'>
		...
		<tr>
			...
			<td valign='top'>
				...
				<tr>
					<td>Alt Cost</td>
					<td><input name='alt_total_cost' id='alt_total_cost' style='text-align:right;background-color:#ffa569' class='xlocked'></td>
				</tr>
				<tr>
					<td>Alt Profit $</td>
					<td><input name='profit_alt' id='profit_alt' style='text-align:right;background-color:#8fff8f;' class='xlocked'></td>
				</tr>
				<tr>
					<td colspan='4'>
						<hr>
				...
				<? if($rate_adjustment > 0 || $rate_adjustment < 0) { ?>
     				<tr>
     					...
     					<td valign='top'>
     						<?  
     						echo "<span style='font-weight:bold; background-color:purple; color:#FFFFFF;'>+ ".number_format(($rate_adjustment * 100),2)."% (<span id='qr_adjustment_val'></span>)</span>";     											
     						?>
     					</td>
     				</tr>
     				<tr>
     					<td valign='top'>Ask Customer For</td>
     					<td valign='top'>
     						<input name='quote_rate_adjustment' id='quote_rate_adjustment' class='xlocked' style='text-align:right;font-weight:bold; background-color:purple; color:#FFFFFF;'>
     					</td>
     				</tr>
				<? } ?>
				<tr>
...
<script type='text/javascript'>
	var qrate_percent=<?=$rate_adjustment ?>;
    ...
    var labor_per_hour = parseFloat(<?= $defaultsarray['labor_per_hour']?>);
	...
	$('#labor_per_hour').val(formatCurrency(labor_per_hour));
	...
	function team_driver_change(flag)
	{
		mrr_tm_flag=flag;
	...
	function load_driver_history(driver_id) 
    {
		...
		tm_driver_flag=0;
		if( $('#team_drivers').val()==1 || mrr_tm_flag > 0)	
		{
			tm_driver_flag=1;
		 ...
		 $.ajax({
		   ...
		   success: function(xml) {
               ...
               if(parseInt(driver_id) > 0)
               {
                   ...
                   $('#labor_per_mile').val(formatCurrency(pay_per_mile));
                   $('#labor_per_hour').val(formatCurrency(pay_per_hour));

                   //$('#labor_per_mile_holder').html("("+formatCurrency($(xml).find('PayPerMile').text())+")");
                   //$('#labor_per_hour_holder').html("("+formatCurrency($(xml).find('PayPerHour').text())+")");
                   calc_all();
               }
               else 
               {
                   pay_per_hour = parseFloat(<?= $defaultsarray['labor_per_hour']?>);
                                      
                   $('#labor_per_mile').val(formatCurrency(labor_per_mile));
                   $('#labor_per_hour').val(formatCurrency(pay_per_hour));                   
                   
                   //$('#labor_per_mile_holder').html("("+formatCurrency(labor_per_mile)+")");
                   //$('#labor_per_hour_holder').html("("+formatCurrency(labor_per_hour)+")");
                   calc_all();
               }
			/*
			if(tm_driver_flag==1)
			{
				pay_per_mile = $(xml).find('PayPerMileTeam').text();
				pay_per_hour = $(xml).find('PayPerHourTeam').text();
			}
			else
			{			
				pay_per_mile = $(xml).find('PayPerMile').text();
	...
	function calc_all() {
		if(eval($("input['name=team_drivers']:checked").val())) {
			...
			if($('#labor_per_mile').val()==0)	$('#labor_per_mile').val(formatCurrency(labor_per_mile_team));
		} else {
			// single driver selected
			if($('#labor_per_mile').val()==0)	$('#labor_per_mile').val(formatCurrency(labor_per_mile));
		}
		
		mrr_daily_cost=get_amount($('#daily_cost').val());		//< ? =get_daily_cost($_POST['truck_id'])? >;
		...
		profit_alt=0;
		/*
		var alt_daily_cost = <?=get_daily_cost($_POST['truck_id'])?>;
		total_maint_per_mile_alt = tractor_maint_per_mile + trailer_maint_per_mile;
		
		total_per_mile_alt = get_amount($('#labor_per_mile').val()) + total_maint_per_mile_alt + fuel_per_mile;
		breakeven_otr_alt = total_per_mile_alt * use_miles + deadhead_cost + (alt_daily_cost * get_amount($('#days_run_otr').val()));
		
		alt_total_cost=breakeven_otr_alt + breakeven_hourly + variable_expenses_total;
		profit_alt = bill_customer - alt_total_cost;
		*/
		...
		mrr_qr_val=parseFloat(bill_customer) * parseFloat(qrate_percent);
		$('#qr_display_val').html(formatCurrency(mrr_qr_val));
		$('#qr_adjustment_val').html(formatCurrency(mrr_qr_val));
		mrr_qr_tot=parseFloat(bill_customer) + parseFloat(mrr_qr_val);
		$('#quote_rate_display').val(formatCurrency(mrr_qr_tot));
		$('#quote_rate_adjustment').val(formatCurrency(mrr_qr_tot));
		...
		$('#profit_alt').val(formatCurrency(profit_alt));
		$('#alt_total_cost').val(formatCurrency(alt_total_cost));
	...
	function calc_run() {
		...
		//build_run......use this below for AJAX if PC Miler in use...
		 ...
		 $.ajax({
		   ...
		   success: function(xml) {
		   		total_miles = get_amount($(xml).find('Miles').text()).toFixed(0);
				$('#pcm_loaded_miles').val(total_miles);
				//$('#loaded_miles').val(total_miles);
				...
				$(xml).find("StopDistance").each(function() {
					...
					while(true) {
						if($('#stop_'+stopcounter).val() == undefined || $('#stop_'+stopcounter).val() != '') break;
						stopcounter++;
					}
					miles = parseFloat($(this).text());
					total_miles += miles;
					$('#miles_'+stopcounter).html(miles.toFixed(0));
					$('#total_'+stopcounter).html(total_miles.toFixed(0));
					
					update_quotes_stop_miles(stopcounter,miles.toFixed(0));					
				});				
	...
	function delete_quote(id) {
		$.prompt("Are you sure you want to <span class='alert'>delete</span> this quote?", {
				...
				submit: function(v, m, f) {
					if(v) {
						window.location = 'quote.php?del='+id;
					}
				}
			}
		);
