<?
$usetitle="Unit Proximaty Report";  
?>
<? include('header.php') ?>
...
<div style='margin-left:30px;margin-top:10px;display:inline;float:left;clear:right'>
	<h3>Closest Unit</h3><br><span class='alert'><b>Please use the Date From, Date To, numeric Zip-Code, and Radius value in miles.</b></span> All other filters are optional but may narrow the focus.<br>
</div>
<div style='clear:both'></div>
		...
		if($miles <= abs($use_radius) && $dist_mode!="N/A")
		{
			...
			{	
				...
				$mrr_map_points.="					
						...
						var truck".$truck_cnt." = new google.maps.Marker({ position: pose".$truck_cnt.",  map: map, icon: '".$icon_path."".$icon_image."', title: 'Truck ".$row['name_truck'].": ".$row['shipper_address1']."' });
					";                                                                                                   //http://www.googlemapsmarkers.com/v1/".$truck_cnt."/00CC00/FFFFFF/0000FF/
			...
			echo "
     			<tr class='load_".$row['load_handler_id']."'>
     				<td><img src='".$icon_path."".$icon_image."' alt='".$truck_cnt."'></td>	
