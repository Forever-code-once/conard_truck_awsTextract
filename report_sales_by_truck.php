<? $usetitle="Sales Report - By Truck"; ?>
<? include('header.php') ?>
<?
 	...
 	{ 
		...
		*/
		...
		$sql = "
			...
			order by trucks.name_truck, load_handler.id, drivers.name_driver_last, drivers.name_driver_first
	...
	<? 
		echo "
			<tr class='odd' style='font-size:14px;font-weight:bold'>
				<td colspan='6' style='font-size:16px;font-weight:bold'>Truck</td>
		...
		while($row = mysqli_fetch_array($data)) 
		{
			...
			if($last_truck_id != $row['truck_id']) 
			{
				
				if($last_truck_id > 0) echo "<tr><td colspan='11'><hr></td></tr>";
				...
				while($row_tmp = mysqli_fetch_array($data)) 
				{
					...
					$line_array = get_line_amount($row_tmp);
				...
				echo "
					<tr class='odd' style='font-size:14px;font-weight:bold'>
						<td colspan='6' style='font-size:16px;font-weight:bold'>Truck: <a href='admin_trucks.php?id=$row[truck_id]' target='view_truck_$row[truck_id]'>$row[name_truck]</a></td>
						...
						<td colspan='2'><i><b>Sales may be split across trucks</b></i></td>
			...
			$line_array = get_line_amount($row);
				...
				while($row_stop = mysqli_fetch_array($data_stops)) 
				{
					echo "
						<tr style='background-color:#e2ffe4'>
							...
							<td colspan='6' nowrap>$row_stop[shipper_address1] $row_stop[shipper_city], $row_stop[shipper_state] $row_stop[shipper_zip]</td>
