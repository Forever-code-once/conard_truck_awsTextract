...
<?
	...
	//$_POST['actual_rate_fuel_surcharge']
	//$_POST['actual_fuel_surcharge_per_mile']
	
	if(!isset($_GET['auto_save_trigger']))		$_GET['auto_save_trigger']=0;
	
	//if(isset($_POST['auto_save_trigger']) && $_POST['auto_save_trigger'] > 0)		$_GET['auto_save_trigger']=0;
	
	if(isset($_GET['load_id'])) $_POST['load_id'] = $_GET['load_id'];
	if(!isset($_POST['load_id'])) $_POST['load_id'] = 0;

	if(!isset($_GET['resend_edi_file']))		$_GET['resend_edi_file']=0;
	$_GET['resend_edi_file']=(int) $_GET['resend_edi_file'];
	...
	$mrr_load_id=$_POST['load_id'];			//$_GET['resend_edi_file']
	
	$current_payroll_bonus_hours=mrr_get_default_variable_setting('payroll_bonus_min_reg_hours');
	...
	if(isset($_GET['dup_id'])) 
	{
		...
		$sql = "
			insert into load_handler
				...
				lynnco_edi_input_file,
				lynnco_edi_output_file,
			...
			values (now(),
				...
				'".sql_friendly($current_payroll_bonus_hours)."',						
	...
	if(isset($_POST['customer_id']) && $_POST['customer_id'] > 0) {
		...
		*/
		...
		$_POST['fedex_edi_invoice_mileage']=str_replace("$","",$_POST['fedex_edi_invoice_mileage']);
		$_POST['fedex_edi_invoice_mileage']=str_replace(",","",$_POST['fedex_edi_invoice_mileage']);
		$_POST['fedex_edi_invoice_mileage']=str_replace(".","",$_POST['fedex_edi_invoice_mileage']);
		$_POST['fedex_edi_invoice_mileage']=(int) $_POST['fedex_edi_invoice_mileage'];
		
		$_POST['lynnco_edi_invoice_mileage']=str_replace("$","",$_POST['lynnco_edi_invoice_mileage']);
		$_POST['lynnco_edi_invoice_mileage']=str_replace(",","",$_POST['lynnco_edi_invoice_mileage']);
		$_POST['lynnco_edi_invoice_mileage']=str_replace(".","",$_POST['lynnco_edi_invoice_mileage']);
		$_POST['lynnco_edi_invoice_mileage']=(int) $_POST['lynnco_edi_invoice_mileage'];
		
		$sql = "
			update load_handler set
				auto_save_requested=0,
				customer_id = '".sql_friendly($_POST['customer_id'])."',
				invoice_number = '".sql_friendly($_POST['invoice_number'])."',				
				".(($_POST['linedate_invoiced'] != '' && trim($_POST['invoice_number'])!="") ? "sicap_invoice_number = '".sql_friendly($_POST['invoice_number'])."'," : "")."
				...
				special_instructions = '".sql_friendly(trim($_POST['special_instructions']))."',
				commodity = '".sql_friendly(trim($_POST['commodity']))."',
				load_available = '".(isset($_POST['load_available']) ? '1' : '0')."',
				load_detention = '".(isset($_POST['load_detention']) ? '1' : '0')."',
				...
				misc_detention = '".($_POST['misc_detention'] == '' ? '0' : sql_friendly(money_strip($_POST['misc_detention'])))."',
				
				fedex_edi_invoice_mileage = '".sql_friendly($_POST['fedex_edi_invoice_mileage'])."',
				lynnco_edi_invoice_mileage = '".sql_friendly($_POST['lynnco_edi_invoice_mileage'])."',
				
				alt_tracking_email= '".sql_friendly(trim($_POST['alt_tracking_email']))."',
		...
		{
			$pn_auto_dispatch_link="peoplenet_interface.php?find_load_id=".$edit_id."&auto_run=1&cs=2";
		}
		...
		$skip_pc_miler=0;
		if(isset($_GET['auto_save_trigger']) && $_GET['auto_save_trigger'] > 0)		$skip_pc_miler=1;
		if(isset($_GET['auto_save_close']) && $_GET['auto_save_close'] > 0)			$skip_pc_miler=1;
		
		update_origin_dest($edit_id,$skip_pc_miler);		
		...
		if(isset($_POST['auto_save_trigger']) && $_POST['auto_save_trigger'] > 0)
		{
			$_GET['auto_save_trigger']=0;
			$_POST['auto_save_trigger']=0;	
			
			$mrr_master_load_editor_flag="&auto_save_close=1";
		}
		
		
		if(substr_count($extra_param,"auto_save_trigger") > 0 || substr_count($extra_param,"auto_save_close") > 0 || substr_count($extra_param,"auto_run") > 0)
		{
			$mrr_use_geotab=0;		//skip this for the auto routines...
		}
		
		//$mrr_use_geotab=0;		//KILL SWITCH TO TURN OFF THE GEOTAB WorkFlow...preplanned load level.  Use the Dispatch on add_entry_truck.php.
		if($mrr_use_geotab > 0 && isset($_POST['preplan']))
		{
			$driver_id_gt=(int) $_POST['preplan_driver_id'];
			$truck_id=mrr_find_geotab_driver_by_id($driver_id_gt,1);
			$truck2_id=mrr_find_geotab_driver_by_id($driver_id_gt,2);
									
			if($truck_id > 0)
			{
				$truck_device_id=mrr_find_geotab_truck_id_by_id($truck_id);
				if(trim($truck_device_id)!="")
				{
					$dres=mrr_send_geotab_complete_preplan_load($edit_id,$truck_id,1,0,1);
               		/*
               		echo "
               ...
               elseif($truck2_id > 0)
               {
               	$truck_device_id=mrr_find_geotab_truck_id_by_id($truck2_id);
				if(trim($truck_device_id)!="")
				{
					$dres=mrr_send_geotab_complete_preplan_load($edit_id,$truck2_id,1,0,1);
               		/*
               		echo "
               			<br><h3>Testing PREPLANNED Load ".$load_id." for Truck ID ".$truck_id.":</h3>
               				<br>Truck ID: ".$dres['truck_id']."
		...
		{
			//kill switch only forthe reloaded page, but just to see the GeoTab stuff.
		}
		else
		{
	...
	if($_POST['load_id'] > 0) 
	{
		/* Get stop list for select box only*/
     	...
     	$data_stopper = simple_query($sql);
          ...
          mrr_repair_gps_points_on_load($_POST['load_id']);
        ...
        $bill_rate_edi=mrr_repair_koch_edi_bill_rate($_POST['load_id']);
		...
		$_POST['geotab_load_id']=trim($row['geotab_load_id']);
		$_POST['geotab_msg_id']=trim($row['geotab_load_msg_id']);
		
		$_POST['alt_tracking_email']=trim($row['alt_tracking_email']);
		
		$_POST['load_detention']=$row['load_detention'];
		...
		$_POST['fedex_edi_invoice_mileage']=$row['fedex_edi_invoice_mileage'];
		$_POST['fedex_edi_error_log']=trim($row['fedex_edi_error_log']);
		
		$_POST['lynnco_edi_invoice_mileage']=$row['lynnco_edi_invoice_mileage'];
		$_POST['lynnco_edi_error_log']=trim($row['lynnco_edi_error_log']);
		...
		//resend the EDI Response file... added for KOCH LOGISTICS.  990 is the response file fo each EDI load.
		if(isset($_GET['resend_edi_file']) && (int) $_GET['resend_edi_file']==990 && (int) $_POST['load_id'] > 0)
		{	
			...
			if(send_koch_edi('/Inbox/', $in_path."/out/", $file_name, $_POST['load_id'])) 
			{
				// log the upload
				$sqlu = "
					update load_handler set
		...
		if((int) $_POST['fedex_edi_invoice_mileage'] > 0)
		{
			//$_POST['actual_miles'] = $_POST['fedex_edi_invoice_mileage'];
			//$_POST['actual_deadhead_miles'] = 0;
		}
		if((int) $_POST['lynnco_edi_invoice_mileage'] > 0)
		{
			//$_POST['actual_miles'] = $_POST['lynnco_edi_invoice_mileage'];
			//$_POST['actual_deadhead_miles'] = 0;
		}
	}
	...
	if($_POST['load_id'] > 0 && $auto_update_pn==1)
	{
		if($send_pn_link > 0)
		{
			$pn_auto_dispatch_link="peoplenet_interface.php?find_load_id=".$_POST['load_id']."&auto_run=1&cs=1";
		}
	}
	...
	if(!isset($_POST['load_detention']))			$_POST['load_detention']=0;
	...
	if(!isset($_POST['fedex_edi_invoice_mileage']))		    $_POST['fedex_edi_invoice_mileage']=0;
	if(!isset($_POST['lynnco_edi_invoice_mileage']))		$_POST['lynnco_edi_invoice_mileage']=0;
	
	if(!isset($_POST['alt_tracking_email']))			    $_POST['alt_tracking_email']="";		
    ...
    if(!isset($_POST['needs_appt'])) 			            $_POST['needs_appt'] = 0;
	if(!isset($_POST['appointment_window'])) 			    $_POST['appointment_window'] = 0;
	...
	if($_GET['load_id'] > 0)
    {
        mrr_user_page_editing_del(1,$_GET['load_id'],$_SESSION['user_id']);
        mrr_user_page_editing_add(1,$_GET['load_id'],$_SESSION['user_id'],"manage_load.php?load_id=".$_GET['load_id']."");
     
        $mrr_show_use_warning=mrr_user_page_editing_finder(1,$_GET['load_id'],0,$_SESSION['user_id']);     //$_SESSION['user_id']
    }
	if($_GET['auto_save_trigger'] > 0)      mrr_user_page_editing_del(1,$_GET['load_id'],$_SESSION['user_id']);
?>
<? $no_header = 1 ?>
<? include('header.php') ?>
<table>
<tr>
	<td valign='top'>
		<div class='nav_bar' style='width:100%;position:fixed;top:0px;left;0px;margin:0px 0 0 -3px;font-family:arial;font-size:12px'>
			...
			<div class='toolbar_button' onclick='save_load_handler(0)'>
				<div><img src='images/file.png'></div>
				<div>Save</div>
			</div>
			<span style='<?=($_GET['load_id']==0 ? "display:none;" : "")?>' class='loaded_load'>
		...
		<input type='hidden' name='auto_save_trigger' id='auto_save_trigger' value='<?=$_GET['auto_save_trigger'] ?>'>
				<!---
             ...
             {
                  //echo "<h2>Warning: This Load is opened by the following:</h2><br>".$mrr_show_use_warning."<br>";
		...
		<tr>
			...
			<td width='50%'>
				...
				if($_POST['preplan'])
				{
					if($_POST['geotab_load_id']!="" && $_POST['geotab_load_id']!="0")
					{
						echo "&nbsp; &nbsp; &nbsp; <span style='color:#00cc00;' title='GeoTab Route ID is ".$_POST['geotab_load_id'].".'><i><b>PREPLAN GeoTab Workflow Route Sent.</b></i></span>";	
					}
					else
					{
						echo "&nbsp; &nbsp; &nbsp; <span style='color:#cc0000;' title='GeoTab Route ID is ".$_POST['geotab_load_id'].".'><i><b>PREPLAN GeoTab Workflow NOT Sent.</b></i></span>";	
					}	
					
					if($_POST['geotab_msg_id']!="" && $_POST['geotab_msg_id']!="0")
					{
						echo "&nbsp; &nbsp; &nbsp; <span style='color:#00cc00;' title='GeoTab Msg ID is ".$_POST['geotab_msg_id'].".'><i><b>PREPLAN GeoTab Msg Sent.</b></i></span>";	
					}
					else
					{
						echo "&nbsp; &nbsp; &nbsp; <span style='color:#cc0000;' title='GeoTab msg ID is ".$_POST['geotab_msg_id'].".'><i><b>PREPLAN GeoTab Msg NOT Sent.</b></i></span>";	
					}				
				}
				else
				{
					echo "&nbsp; &nbsp; &nbsp; <span style='color:#aaaaaa;'><i><b>See Dispatch(es) for GeoTab Workflow</b></i></span>";	
				}	
				?>
			</td>
		</tr>
		...
		<tr>
			<td valign='top'><label for='load_detention'>Detention Needed</label></td>
			<td valign='top'>
				<input type='checkbox' name='load_detention' id='load_detention' <?=($_POST['load_detention'] > 0 ? 'checked' : '')?>> <?= show_help('manage_load.php','load_detention checkbox') ?>
                 <? if($_POST['load_id'] > 0) { ?>                      
                      <div id="show_detention_notes"></div>
                 <? } ?>
			</td>
			<td>
                <? if($_POST['load_id'] > 0) { ?>
                     <div style="float:left; margin-right:10px;">
                        <input type='button' name='detention_note_remover' id='detention_note_remover' value='Clear Note' onClick="mrr_clear_detention_note();"> 
                     </div>
                    
                    <div style="float:right; margin-left:10px;">
                        <input type='button' name='detention_note_adder' id='detention_note_adder' value='Add Note' onClick="mrr_add_detention_note(<?=$_POST['load_id'] ?>);">
                    </div>
                     
                    <input type='text' name='detention_note' id='detention_note' value="" style="width:400px;">                                        
                <? } ?>
            </td>
		</tr>
		<tr>
					...
					<td valign='top'>
						<div id='customer_id_holder'>
							...
							while($row_customers = mysqli_fetch_array($data_customers)) 
							{
								...
								echo " dirt_bags_flag='".$row_customers['dirt_bags_flag']."'";
								echo " document_75k_received='".$row_customers['document_75k_received']."'";
								echo " document_75k_exempt='".$row_customers['document_75k_exempt']."'";	
								
								$date_75k1=date("Ymd",strtotime($row_customers['linedate_document_75k']));		if($row_customers['linedate_document_75k']<"2010-01-01 00:00:00")		$date_75k1="";
								$date_75k2=date("Ymd",strtotime($row_customers['linedate_expires_75k']));		if($row_customers['linedate_expires_75k'] <"2010-01-01 00:00:00")		$date_75k2="";
								
								if($date_75k2=="" && $date_75k1!="")
								{
									$date_75k2=date("Ymd",strtotime("+1 year",strtotime($row_customers['linedate_document_75k'])));	
								}								
								
								echo " linedate_document_75k='".$date_75k1."'";
								echo " linedate_expires_75k='".$date_75k2."'";
                                 
                                $date_75k3=date("Ymd",strtotime($row_customers['linedate_renewal_75k']));		if($row_customers['linedate_renewal_75k'] <"2010-01-01 00:00:00")		$date_75k3="";
                                $date_chck=date("Ymd",strtotime($row_customers['linedate_credit_check']));		if($row_customers['linedate_credit_check'] <"2010-01-01 00:00:00")		$date_chck="";
                                 
                                if($date_75k3=="" && $date_75k2!="")
                                {
                                     $date_75k3=date("Ymd",strtotime("+6 month",strtotime($row_customers['linedate_expires_75k'])));
                                }
                                                                
                                echo " linedate_renewal_75k='".$date_75k3."'";
                                echo " cust_credit_check='".$row_customers['cust_credit_check']."'";
                                echo " linedate_credit_check='".$date_chck."'";
                                 
                                $date_chck_exp=date("Ymd",strtotime($row_customers['linedate_credit_check_exp']));		if($row_customers['linedate_credit_check_exp'] <"2010-01-01 00:00:00")		$date_chck_exp="";
                                 
                                echo " linedate_credit_check_exp='".$date_chck_exp."'";
        ...
        <tr>
            ...
            <td colspan='10'><input name='load_number' id='load_number' value='<?=$_POST['load_number']?>' class='input_medium' onBlur='mrr_lading_number_search();'></td>
        </tr>
		</table>
		...
		<tr id='stop_holder_new'>
			<td valign='top' style='border:1px black solid' colspan='2' class='admin_menu3'>
                ...
                <tr>
                    <td>Appointment Needed</td>
                    <td>
                        <input type='checkbox' name='needs_appt' id='needs_appt' <?=($_POST['needs_appt'] > 0 ? 'checked' : '')?>> <?= show_help('manage_load.php','needs_appt') ?>
                    </td>
                </tr>    
				<tr>
				...
				<tr>
					<td valign='top'>Fill as/Relay</td>
					<td valign='top'>
						<?
						if($terminal['name']!="" && 1==2)
						{
							echo "
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						...
						?>
                        <select name='stop_relay_id' id='stop_relay_id' onChange='mrr_fill_stop_as_relay();'>
                            <option value='0'>None</option>
                             <?
                                //additional stops go here...
                                $sqlr="
                                    select * 
                                    from relay_stops 
                                    where deleted=0 
                                ...
                                {
                                     echo "<option value='".$rowr['id']."'>".$rowr['relay_name']." - ".$rowr['relay_city'].", ".$rowr['relay_state']." ".$rowr['relay_zip']."</option>";
     
                                     $stop_relay_array.="
                                        relay_name[".$rowr['id']."]=\"".trim($rowr['relay_name'])."\";
                                        relay_addr1[".$rowr['id']."]=\"".trim($rowr['relay_address1'])."\";
                                        relay_addr2[".$rowr['id']."]=\"".trim($rowr['relay_address2'])."\";
                                        relay_city[".$rowr['id']."]=\"".trim($rowr['relay_city'])."\";
                                        relay_state[".$rowr['id']."]=\"".trim($rowr['relay_state'])."\";
                                        relay_zip[".$rowr['id']."]=\"".trim($rowr['relay_zip'])."\";
                                        relay_phone[".$rowr['id']."]=\"".trim($rowr['relay_phone'])."\";
                                    ";
                                }
                             ?>
                        </select>
				...
				*/ ?>
				...
				<div width='820'>
					...
					Special Instructions  (Internal)<?= show_help('manage_load.php','Special Instructions for Stop') ?>
					<br>
		...
		<tr>
			<td valign='top'>
     		...
     		<? if($use_new_uploader > 0) { ?>
     			...
     			<center>
          			<iframe src="mrr_uploader_hack.php?section_id=8&id=<?=$_GET['load_id']?>" width='550' height='80' border='0' style='border:#000000 solid 0px; background-color:#ffffff;margin-left:5px;'>
          			</iframe> 
          		</center>
     		...
     		<tr>
     			<td colspan='2'>
     				<b>Commodity: (for GeoTab)</b> <?= show_help('manage_load.php','commodity') ?> <span style='color:#CC0000;'><b>REQUIRED FIELD!</b></span><br>
     				<input type='text' name='commodity' id='commodity' value='<?=$_POST['commodity']?>' style='width:400px;'>
     				<hr>
     			</td>
     		</tr>
     		<tr>
     			<td colspan='2'>
     				<b>Special Instructions (for GeoTab)</b> <?= show_help('manage_load.php','Special Instructions') ?><br>
     		...
     		<tr>
     			<td colspan='2'>
     				<b>Alternate Tracking E-Mail Address</b> <?= show_help('manage_load.php','alt_tracking_email') ?><br>
     				<input type='text' name='alt_tracking_email' id='alt_tracking_email' value='<?=$_POST['alt_tracking_email']?>' style='width:400px;'>
     				<hr>
     			</td>
     		</tr>     		
     		<tr>
     		...
     		<tr>
     			<td valign='top'>
                    ...
                    */
                    ...
                    <tr>
                        <td valign='top'>
                            Detention <?= show_help('manage_load.php','misc_detention') ?>
                        </td>
                        <td valign='top'>
                            <input name='misc_detention' id='misc_detention' style='text-align:right' value='$<?=number_format($_POST['misc_detention'],2) ?>' title="Use this for Detention amount for invoicing.">
                        </td>
                    </tr>
     				<tr>
     					<td valign='top'>
                            FedEx Invoice Mileage <?= show_help('manage_load.php','fedex_edi_invoice_mileage') ?>
                            <?
                            $fedex_fuel_rate=0;
                            $fedex_route_rate=0;
                            $fedex_inv_total=0;
                            if($_POST['fedex_edi_invoice_mileage'] > 0)
                            {
                                 $fedex_fuel_rate=($_POST['actual_fuel_surcharge_per_mile'] * $_POST['fedex_edi_invoice_mileage']);
                                 $fedex_route_rate=$_POST['actual_bill_customer'] - $fedex_fuel_rate;
                                 $fedex_inv_total=$fedex_route_rate + $fedex_fuel_rate;
                            }
                            if($driver_ooic_rate > 0)
                            {
                                 $est_drive_ooic_pay= ($fedex_inv_total * $driver_ooic_rate / 100);
                                 echo "<br>&nbsp;<br><i>O.O.I.C. Approx. Driver<br>Pay at ".$driver_ooic_rate."% of total<br>$".number_format($est_drive_ooic_pay,2)."</i>";
                            }
                            ?>
                        </td>
     					<td valign='top'>
     						<input name='fedex_edi_invoice_mileage' id='fedex_edi_invoice_mileage' style='text-align:right' value='<?=$_POST['fedex_edi_invoice_mileage'] ?>'>
     						<div align='right'>
          						<br>
                                FedEx Invoice Fuel &nbsp;&nbsp;&nbsp;<b>$<span id='fedex_fuel_rate_disp'><?=number_format($fedex_fuel_rate, 2) ?></span></b>
          						<br>
          						<u>FedEx Invoice Rate &nbsp;<b>$<?=number_format($fedex_route_rate, 2) ?></b></u>
          						<br>
          						FedEx Total Invoiced &nbsp;<b>$<?=number_format($fedex_inv_total, 2) ?></b>
     						</div>
     					</td>
     				</tr>
     				<tr>
     					<td valign='top'>LynnCo Invoice Mileage <?= show_help('manage_load.php','lynnco_edi_invoice_mileage') ?></td>
     					<td valign='top'>
     						<?
     						$lynnco_fuel_rate=0;
     						$lynnco_route_rate=0;
     						$lynnco_inv_total=0;
     						if($_POST['lynnco_edi_invoice_mileage'] > 0)
     						{
     							$lynnco_fuel_rate=($_POST['actual_fuel_surcharge_per_mile'] * $_POST['lynnco_edi_invoice_mileage']);
     							$lynnco_route_rate=$_POST['actual_bill_customer'] - $lynnco_fuel_rate;
     							$lynnco_inv_total=$lynnco_route_rate + $lynnco_fuel_rate;
     						}
     						?>
     						<input name='lynnco_edi_invoice_mileage' id='lynnco_edi_invoice_mileage' style='text-align:right' value='<?=$_POST['lynnco_edi_invoice_mileage'] ?>'>
     						<div align='right'>
          						<br>
                                LynnCo Invoice Fuel &nbsp;&nbsp;&nbsp;<b>$<span id='lynnco_fuel_rate_disp'><?=number_format($lynnco_fuel_rate, 2) ?></span></b>
          						<br>
          						<u>LynnCo Invoice Rate &nbsp;<b>$<?=number_format($lynnco_route_rate, 2) ?></b></u>
          						<br>
          						LynnCo Total Invoiced &nbsp;<b>$<?=number_format($lynnco_inv_total, 2) ?></b>
     						</div>
     					</td>
     				</tr>
     				<tr>
     				...
     				<tr>
     					...
     					<td><input name='actual_bill_customer' id='actual_bill_customer_calc' style='text-align:right;background-color:#8fff8f;' value='' class='rate_field calc_watch xlocked' title="Starts out <?=$_POST['actual_bill_customer'] ?>."></td>
     				</tr>
     				<? 
     				/*
     				<tr>
     			...
     			<td valign='top'>
     				<?
     				if((substr_count($_POST['special_instructions'],"from Fedex EDI") > 0 || substr_count($_POST['special_instructions'],"from EDI file") > 0) && isset($_GET['load_id']) && $_GET['load_id'] > 0)
     				{
     					$edi_received=mrr_fedex_990_display($_GET['load_id']);
     					echo $edi_received;
     					
     					$edi_log=mrr_get_fedex_edi_invoicing_logs((int) $_GET['load_id'],1,500,800);
     					echo $edi_log;
     					
     					if(trim($_POST['fedex_edi_error_log'])!="")
     					{
     						echo "<br><br><b>Last Error Log Entry:</b><br>".$_POST['fedex_edi_error_log']."<br>";
     					}
     				}
     				if(substr_count($_POST['special_instructions'],"from LynnCo EDI") > 0 && isset($_GET['load_id']) && $_GET['load_id'] > 0)
     				{     					
     					$edi_received=mrr_lynnco_990_display($_GET['load_id']);
     					echo $edi_received;
     					
     					$edi_log=mrr_get_lynnco_edi_invoicing_logs((int) $_GET['load_id'],1,1200,800);
     					echo $edi_log;
     					
     					if(trim($_POST['lynnco_edi_error_log'])!="")
     					{
     						echo "<br><br><b>Last Error Log Entry:</b><br>".$_POST['lynnco_edi_error_log']."<br>";
     					}     					
     				}
                    if(substr_count($_POST['special_instructions'],"from Koch EDI") > 0 && isset($_GET['load_id']) && $_GET['load_id'] > 0)
                    {
                         $edi_received=mrr_koch_990_display($_GET['load_id']);
                         echo $edi_received;
                    
                         $edi_log=mrr_get_koch_edi_invoicing_logs((int) $_GET['load_id'],1,1200,800);
                         echo $edi_log;
                    
                         if(trim($_POST['koch_edi_error_log'])!="")
                         {
                              echo "<br><br><b>Last Error Log Entry:</b><br>".$_POST['koch_edi_error_log']."<br>";
                         }
                    }
     				?>     					
     			</td>	
...
<script type='text/javascript'>
	...
	var debug = <?=($_SERVER['REMOTE_ADDR'] == '70.90.229.29' ? 1 : 0)?>;
	
	var mrr_now2 = '<?= date("Ymd"); ?>' ;
	var min_linedate_expires_75k='<?= date("Ymd",strtotime("-14 days", time())) ?>';
	
	function mrr_load_detention_notes()
    {
        if(load_id==0)      return '';
	...
	function mrr_clear_detention_note()
    {
        $('#detention_note').val('');
    }
	function mrr_add_detention_note(loadid)
    {
        if(parseInt(loadid)==0)     loadid=load_id;
        
        if(parseInt(loadid)==0)
        {
            $.prompt("<h2>Error: You must save the load to write a detention note.");
            return false;
        }
        
        var dnote=$('#detention_note').val();

        $.ajax({
            type: "POST",
            url: "ajax.php?cmd=mrr_add_detention_note",
            data: {"note":dnote,
                "load_id":loadid},
            dataType: "xml",
            cache:false,
            success: function(xml) {
                rslt = parseInt($(xml).find('rslt').text());
                if(rslt > 0)
                {
                    $.noticeAdd({text: "New load Detention Note added successfully. :)"});
                    mrr_clear_detention_note();
                    mrr_load_detention_notes();
                }
                else
                {
                    $.prompt("Failed to make Detention Note.  :(");
                }
            }
        });
    }
    ...
    {
        mrr_update_user_pg_usage(close_after_refresh);
        //parent_window_refresh(close_after_refresh);
    }
    function mrr_update_user_pg_usage(close_after_refresh)
    {
        ...
        $.ajax({
            ...
            cache:false,
            success: function(xml) {                
                //$.prompt("<h2>Warning: This Load is opened by the following:</h2><br>User is "+$(xml).find('User').text()+".<br>");                
                $.noticeAdd({text: "Thanks for closing out Load "+load_id+"."});
    ...
    <? if(trim($mrr_show_use_warning)!="") { ?>
        $().ready(function() {
            ...
            $.prompt("<h2>Warning: This Load is opened by the following:</h2><br><?= $mrr_show_use_warning ?><br>");
        });        
    <? } ?>
	...
	<?=$stop_relay_array ?>
    function mrr_fill_stop_as_relay()
    {
        var relay_id=parseInt($('#stop_relay_id').val());
        ...
        if(relay_id > 0) 
        {
            mrr_hub_phone = relay_phone[relay_id];
            mrr_hub_name = relay_name[relay_id];
            mrr_hub_addr = relay_addr1[relay_id];
            mrr_hub_addr2 = relay_addr2[relay_id];
            mrr_hub_city = relay_city[relay_id];
            mrr_hub_state = relay_state[relay_id];
            mrr_hub_zip = relay_zip[relay_id];
        }
        
        $('#shipper').val(mrr_hub_name);
        $('#origin_address1').val(mrr_hub_addr);
        $('#origin_address2').val(mrr_hub_addr2);
        $('#origin_city').val(mrr_hub_city);
        $('#origin_state').val(mrr_hub_state);
        $('#origin_zip').val(mrr_hub_zip);
        $('#origin_phone').val(mrr_hub_phone);
	...
	function save_load_handler(bypasser) {
		...
		if(bypasser == 0) 
		{ 
			if($('#commodity').val() == '' || $('#commodity').val() == ' ') {
				$.prompt("You must enter the 'Commodity' for GeoTab Shipment Logs to save this load.");
				return false;
			}		
		}	
	...
	$('#customer_id').change(function() {
		...
		dirt_bags_flag = $('#customer_id option:selected').attr('dirt_bags_flag');
		...
		linedate_document_75k= $('#customer_id option:selected').attr('linedate_document_75k');	
		linedate_expires_75k= $('#customer_id option:selected').attr('linedate_expires_75k');
		
		linedate_renewal_75k= $('#customer_id option:selected').attr('linedate_renewal_75k');
        cust_credit_check= $('#customer_id option:selected').attr('cust_credit_check');
        linedate_credit_check= $('#customer_id option:selected').attr('linedate_credit_check');
        linedate_credit_check_exp= $('#customer_id option:selected').attr('linedate_credit_check_exp');
		...
		{
			$.prompt("<span class='alert'>Danger! Danger!:</span> '"+$('#customer_id option:selected').text()+"' is missing the <span class='alert'>75K Bond Document</span> - no new loads can be created for this customer. (Please see Dale or Justin to override.)");
			$('#customer_id').val(0);
			return false;	
		}
		if(document_75k_received=='1' && document_75k_exempt=='0')
		{
			if(linedate_document_75k=='')
			{
				$.prompt("<span class='alert'>Warning!:</span> '"+$('#customer_id option:selected').text()+"' is missing the <span class='alert'>75K Bond Inception Date</span> - no new loads can be created for this customer. (Please see Dale or Justin to override.)");
				$('#customer_id').val(0);
				return false;	
			}
            if(linedate_expires_75k=='')
            {
                $.prompt("<span class='alert'>Warning!:</span> '"+$('#customer_id option:selected').text()+"' is missing the <span class='alert'>75K Bond Expiration Date</span> - no new loads can be created for this customer. (Please see Dale or Justin to override.)");
                $('#customer_id').val(0);
                return false;
            }
            if(linedate_expires_75k!='' && linedate_expires_75k < mrr_now2)
            {
                $.prompt("<span class='alert'>Warning!:</span> '"+$('#customer_id option:selected').text()+"' has an expired date for <span class='alert'>75K Bond Expiration Date</span> - no new loads can be created for this customer. (Please see Dale or Justin to override.)");
                $('#customer_id').val(0);
                return false;
            }
            if(linedate_expires_75k!='' && linedate_expires_75k >= mrr_now2 && linedate_expires_75k < min_linedate_expires_75k)
            {
                $.prompt("<span class='alert'>Warning!:</span> '"+$('#customer_id option:selected').text()+"' will soon expire or needs renewal for the <span class='alert'>75K Bond Expiration Date</span> - this is only a warning.");
                //$('#customer_id').val(0);
                //return false;	
            }
			/*
            if(linedate_renewal_75k!='' && linedate_renewal_75k < mrr_now2)
            {
                $.prompt("<span class='alert'>Warning!:</span> '"+$('#customer_id option:selected').text()+"' should be RENEWED for <span class='alert'>75K Bond Renewal Date</span> - no new loads can be created for this customer. (Please see Dale or Justin to override.)");
                $('#customer_id').val(0);
                return false;
            }
            else 
            {
                if(linedate_renewal_75k!='' && linedate_renewal_75k >= mrr_now2 && linedate_renewal_75k < min_linedate_expires_75k)
                {
                    $.prompt("<span class='alert'>Warning!:</span> '"+$('#customer_id option:selected').text()+"' should be RENEWED SOON for <span class='alert'>75K Bond Renewal Date</span> - this is only a warning.");
                    //$('#customer_id').val(0);
                    //return false;
                }
                else                     
                {
                    if(linedate_expires_75k!='' && linedate_expires_75k < mrr_now2)
                    {
                        $.prompt("<span class='alert'>Warning!:</span> '"+$('#customer_id option:selected').text()+"' has an expired date for <span class='alert'>75K Bond Expires Date</span> - no new loads can be created for this customer. (Please see Dale or Justin to override.)");
                        $('#customer_id').val(0);
                        return false;
                    }
                    if(linedate_expires_75k!='' && linedate_expires_75k >= mrr_now2 && linedate_expires_75k < min_linedate_expires_75k)
                    {
                        $.prompt("<span class='alert'>Warning!:</span> '"+$('#customer_id option:selected').text()+"' will soon expire or need renewal for the <span class='alert'>75K Bond Expires Date</span> - this is only a warning.");
                        //$('#customer_id').val(0);
                        //return false;	
                    }
                }
        ...
        if(cust_credit_check=='1')
        {
            if(linedate_credit_check_exp=='')
            {
                $.prompt("<span class='alert'>Warning!:</span> '"+$('#customer_id option:selected').text()+"' is missing the <span class='alert'>Credit Check Expiration Date</span> - no new loads can be created for this customer. (Please see Dale or Justin to override.)");
                $('#customer_id').val(0);
                return false;
            }
            if(linedate_credit_check_exp!='' && linedate_credit_check_exp < mrr_now2)
            {
                $.prompt("<span class='alert'>Warning!:</span> '"+$('#customer_id option:selected').text()+"' should be rechecked for the <span class='alert'>Credit Check Expiration Date</span> - no new loads can be created for this customer. (Please see Dale or Justin to override.)");
                $('#customer_id').val(0);
                return false;
            }
            if(linedate_credit_check_exp!='' && linedate_credit_check_exp >= mrr_now2 && linedate_credit_check_exp < min_linedate_expires_75k)
            {
                    $.prompt("<span class='alert'>Warning!:</span> '"+$('#customer_id option:selected').text()+"' should be rechecked SOON for <span class='alert'>Credit Check Expiration Date</span> - this is only a warning.");
                    //$('#customer_id').val(0);
                    //return false;
            }
            /*
            if(linedate_credit_check!='' && linedate_credit_check < mrr_now2)
            {
                $.prompt("<span class='alert'>Warning!:</span> '"+$('#customer_id option:selected').text()+"' should be RENEWED for <span class='alert'>Credit Check Date</span> - no new loads can be created for this customer. (Please see Dale or Justin to override.)");
                $('#customer_id').val(0);
                return false;
            }
            else
            {
                if(linedate_credit_check!='' && linedate_credit_check >= mrr_now2 && linedate_credit_check < min_linedate_expires_75k)
                {
                    $.prompt("<span class='alert'>Warning!:</span> '"+$('#customer_id option:selected').text()+"' should be RENEWED SOON for <span class='alert'>Credit Check Date</span> - this is only a warning.");
                    //$('#customer_id').val(0);
                    //return false;
                }
            }
		...
		*/
		...
		{
			$.prompt("<span class='alert'>Danger! Danger!:</span> '"+$('#customer_id option:selected').text()+"' has a <span class='alert'>credit hold</span> - no new loads can be created for this customer. (Please see Dale or Justin to override.)");
			$('#customer_id').val(0);
			return false;
		}
		else
		...
		if(dirt_bags_flag == '1') 
		{
			$.prompt("<span class='alert'>DIRT BAGS!:</span> '"+$('#customer_id option:selected').text()+"' is or is full of <span class='alert'>DIRT BAGS!</span> - no new loads can be created for this customer. (Please see Dale or Justin to remove this.)");
			$('#customer_id').val(0);
			return false;
		}
		...
		//alert('Weekday='+weekday+'.');
			 ...
			 $.ajax({
			   ...
			   success: function(xml) {
				...
				fuel_per_mile = parseFloat($(xml).find('FuelPerMile').text());
				fuel_per_mile_used = parseInt($(xml).find('FuelPerMileUsed').text());
				fuel_per_mile_auto = parseInt($(xml).find('FuelPerMileAuto').text());
				
				if(load_id == 0 && fuel_per_mile_used > 0 && fuel_per_mile_auto > 0) {
        ...
        if($('#needs_appt').attr('checked'))	        mrr_checked_appt_needed=1;	
		 ...
		 $.ajax({
		   ...
		   data: {load_id:load_id,
                ...
                needs_appt:mrr_checked_appt_needed,
		...
		loaded_stop = stop_id;
		 ...
		 $.ajax({
		   ...
		   success: function(xml) {
			...
			$('#geotab_api_msg_url').html($(xml).find('GeotabAPImsgURL').text());
			
			mrr_appt_win='';
            mrr_appt_need='';
            if($(xml).find('ApptWindow').text() > 0)	mrr_appt_win=' checked';
			if($(xml).find('ApptNeeded').text() > 0)	mrr_appt_need=' checked';
			
			$('#needs_appt').attr('checked',mrr_appt_need);
	...
	*/
	...
	if(isset($_GET['load_id']) && $_GET['load_id'] > 0) 
	{
		//echo " create_upload_section('#upload_section', 8, $_GET[load_id]); ";
		if($use_new_uploader > 0) 
		{ 
			echo " create_upload_section_alt('#upload_section', 8, $_GET[load_id]); "; 
		}
		else
		{
			echo " create_upload_section('#upload_section', 8, $_GET[load_id]); "; 
	...
	function calc_all() {
        ...
        actual_variable_expenses_total += get_amount($('#misc_detention').val());
		...
		if(get_amount($('#actual_fuel_charge_per_mile').val()) == 0) 
		{
			avg_fuel_per_mile=get_amount($('#actual_fuel_surcharge_per_mile_holder').html());
			$('#actual_fuel_surcharge_per_mile').val(formatCurrency(avg_fuel_per_mile));
		}
		...
		//override the fuel surcharge total if the FedEx or LynnCo Invoice Mileage has been adjusted...swap with what should be on the invoice itself.  Added Jan 2021 by MRR......
		if(parseFloat($('#fedex_edi_invoice_mileage').val())> 0)
        {
            fuel_surcharge_per_mile_total = get_amount($('#fedex_fuel_rate_disp').html());
        }
		if(parseFloat($('#lynnco_edi_invoice_mileage').val())> 0)
        {
            fuel_surcharge_per_mile_total = get_amount($('#lynnco_fuel_rate_disp').html());            
        }
		//............................................
        ...
        if(parseFloat($('#fedex_edi_invoice_mileage').val())> 0)
        {
            //$('#actual_bill_customer').val(formatCurrency(actual_variable_expenses_total + fuel_surcharge_per_mile_total + mrr_flat_fuel_rate));
            mrr_val=get_amount($('#actual_bill_customer_calc').val());
            mrr_update_load_bill_cust_amnt(load_id,mrr_val);
        }
        if(parseFloat($('#lynnco_edi_invoice_mileage').val())> 0)
        {
            //$('#actual_bill_customer').val(formatCurrency(actual_variable_expenses_total + fuel_surcharge_per_mile_total + mrr_flat_fuel_rate));
	...
	function mrr_update_load_bill_cust_amnt(loadid,mrrval)
    {
        $.ajax({
            url: "ajax.php?cmd=mrr_update_load_bill_cust_amnt",
            data: {
                "load_id" : loadid,
                "bill_cust": mrrval
            },
            ...
            success: function(xml) {
                ...
                $.noticeAdd({text: "Success - Actual Bill Customer amount updated from FedEx/LynnCo adjustment."});
            }
        });
    }
	...
	$().ready(function() {
		<?		
		if(!isset($_GET['auto_save_close']))		$_GET['auto_save_close']=0;	
		if(isset($_GET['load_id']) && $_GET['load_id'] > 0 && $_GET['auto_save_trigger'] > 0) {
			echo "save_load_handler(1);"; 
		}
		if(isset($_GET['load_id']) && $_GET['load_id'] > 0 && $_GET['auto_save_close'] > 0) {
			echo "window.close();"; 
		}		
		?>			
	});
