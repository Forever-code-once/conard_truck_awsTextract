...
<?
	$use_title="Conard Insurance Report";
?>
<? include('header.php') ?>
	...
	<tr>
		<td valign='top'>
          	<div style='text-align:left;margin:10px'>
          		<div class='section_heading'>Insurance Report</div>
          ...
          <td valign='top' align='right'>
          	<?
          		...
          		$rfilter->show_filter();
          	?>
          	<br>Archive copies are saved each time the Insurance Report is sent via E-Mail.
          </td>
     </tr>
<? } ?>
<tr>
...
<?
	...
	$excel_filename = "insurance_report".$uuid.".xls";
	$export_file = "";
	$use_excel=1;
	...
	if(isset($_POST['build_report'])) 
	{
		...
		*/
		...
		$sql = "
			select geotab_device_serial, trucks.apu_value, trucks.apu_number, trucks.name_truck, trucks.id,linedate_returned,geotab_device_num,trucks.apu_serial
			from trucks
			where trucks.deleted = 0				
				and trucks.peoplenet_tracking > 0	
				and trucks.geotab_device_serial !='' 
				and trucks.apu_value > 0
				and trucks.insurance_exclude=0
				and trucks.company_admin_vehicle=0	
			order by trucks.name_truck,geotab_device_serial
		";		//trucks.apu_serial
		...
		while($row_truck = mysqli_fetch_array($data_trucks)) {
			...
			if($row_truck['replacement_truck_id'] > 0 && $row_truck['linedate_returned']!="2014-06-30 00:00:00") {
				...
				$vres=mrr_display_equipment_depreciation_final($date_starter,$row_truck['id'],0);		//date, truck ID, trailer ID
				if($vres['cur_equip_value'] > 0)			$e_value=	$vres['cur_equip_value'];
				//if($vres['cur_unit_value'] >=0)			$e_value=	$vres['cur_unit_value'];
				
				if($e_value==0 && $vres['cnt']==1)		$e_value = $row_truck['equipment_value'];
		...
		while($row_val = mysqli_fetch_array($data_interchange)) 
		{
			...
			else 
			{
				...
				$e_value=$row_val['equipment_value'];
     			...
     			//pull value from equipment tracking spreadsheet.... if value is "-1", no record, so use the equipment history value like planned.
     			$vres=mrr_display_equipment_depreciation_final($date_starter,0,$row_val['id']);		//date, truck ID, trailer ID
     			if($vres['cur_equip_value'] >=0)			$e_value=	$vres['cur_equip_value'];
     			//if($vres['cur_unit_value'] >=0)			$e_value=	$vres['cur_unit_value'];
		...
		while($row_val = mysqli_fetch_array($data_trailers)) 
		{
			...
			else 
			{
				...
				$e_value=$row_val['equipment_value'];
     			...
     			if($e_value==0 && $vres['cnt']==1)		$e_value = $row_trailer['equipment_value'];
		...
		while($row_pn = mysqli_fetch_array($data_pn)) 
		{
			...
			else 
			{
				...
				if($found > 0)
				{
					...
					//pull value from equipment tracking spreadsheet.... if value is "-1", no record, so use the equipment history value like planned.
					$vres=mrr_display_equipment_depreciation_final($date_starter,$row_pn['id'],0);		//date, truck ID, trailer ID
					//if($vres['cur_equip_value'] >=0)			$e_value=	$vres['cur_equip_value'];
					if($vres['cur_unit_value'] >=0)			$e_value=	$vres['cur_unit_value'];
		...
		*/
		...
		$physical_damage_liability_cost = money_strip($physical_damage_liability) * money_strip($total_value);
		$cargo_liability_cost = money_strip($cargo_liability) * $billable_trucks;
		/*
		$physical_damage_liability_cost = $total_value * $physical_damage_liability;
		
		
		$total_value)."</td>
				<td>Physical Damage x $physical_damage_liability <span style='color:orange;'>Excluded $".number_format($mrr_excluded,2)."</span></td>
				<td align='right'>$".money_format('', $physical_damage_liability_cost
		*/
		...
		$export_file .= "Insurance report for ".date("M j, Y", $date_start)." to " . date("M j, Y", $date_end)."".chr(9).
          	"".chr(9).
          	"".chr(9).
          	"".chr(9);
     	$export_file .= chr(13);	
     	
     	$export_file .= "Type".chr(9).
          	"Amount".chr(9).
          	"".chr(9).
          	"Coverage Rates".chr(9).
          	"".chr(9).
          	"".chr(9).
          	"".chr(9).
          	"Earned Premium".chr(9);
     	$export_file .= chr(13);	
     	
     	$export_file .= "NU".chr(9).
          	"".$billable_trucks."".chr(9).
          	"".chr(9).
          	"Primary Liabilty x $".number_format(money_strip($primary_liability),3)."".chr(9).
          	"".chr(9).
          	"".chr(9).
          	"".chr(9).
          	"$".number_format($primary_liability_cost,2)."".chr(9);
     	$export_file .= chr(13);	
     	$export_file .= "NU".chr(9).
          	"".$billable_trucks."".chr(9).
          	"".chr(9).
          	"General Liabilty x $".money_format('',money_strip($general_liability))."".chr(9).
          	"".chr(9).
          	"".chr(9).
          	"".chr(9).
          	"$".money_format('', $general_liability_cost)."".chr(9);
     	$export_file .= chr(13);	
     	$export_file .= "STV".chr(9).
          	"$".number_format($total_value,2)."".chr(9).
          	"".chr(9).
          	"Physical Damage x $physical_damage_liability ...Excluded $".number_format($mrr_excluded,2)."".chr(9).
          	"".chr(9).
          	"".chr(9).
          	"".chr(9).
          	"$".number_format($physical_damage_liability_cost,2)."".chr(9);
     	$export_file .= chr(13);	
     	$export_file .= "NU".chr(9).
          	"".$billable_trucks."".chr(9).
          	"".chr(9).
          	"Cargo x $".number_format(money_strip($cargo_liability),3)."".chr(9).
          	"".chr(9).
          	"".chr(9).
          	"".chr(9).
          	"$".number_format($cargo_liability_cost,2)."".chr(9);
     	$export_file .= chr(13);	     	
     	     	
		$export_file .= "".chr(9).
          	"".chr(9).
          	"".chr(9).
          	"Total Earned Premium: $".money_format('',$total_premium)."".chr(9).
          	"".chr(9);
     	$export_file .= chr(13);	
		...
		$report_tab.="
			...
			<tr>
				<td>STV</td>
				<td align='center'>$".number_format($total_value,2)."</td>
				<td>Physical Damage x $physical_damage_liability <span style='color:orange;'>Excluded $".number_format($mrr_excluded,2)."</span></td>
				<td align='right'>$".number_format($physical_damage_liability_cost,2)."</td>
			</tr>
			<tr>
				<td>NU</td>
				<td align='center'>".$billable_trucks."</td>
		...
		//Excel spacer
     	...
     	$export_file .= "Tractor Count: ".$billable_trucks."".chr(9);	
     	$export_file .= chr(13);	
				...
				*/
				...
				$report_tab.= "
					<tr>
						...
						<td>$row_truck[geotab_device_serial]</td>
						<td align='right'>".$mrr_mask1."".($e_value > 0 ? "$".money_format('', $e_value) : "")."".$mrr_mask2."</td>
					</tr>
				";	//<br>".$e_value."
				$mrr_truck_tot+=$e_value;
				$mrr_test_trucks.="<br>".$e_value."";
				...
				$export_file .= "$counter".chr(9).
                    	...
                    	"$row_truck[leased_from]".chr(9).
                    	"".$owner_flag."".chr(9).
                    	"".($row_truck['linedate_aquired'] > 0 ? date("m/d/Y", strtotime($row_truck['linedate_aquired'])) : "")."".chr(9).
                    	"".($row_truck['linedate_returned'] > 0 ? date("m/d/Y", strtotime($row_truck['linedate_returned'])) : "")."".chr(9).
                    	"$row_truck[geotab_device_serial]".chr(9).
                    	"".$mrr_mask1."".($e_value > 0 ? "$".money_format('', $e_value) : "")."".$mrr_mask2."".chr(9);
               	$export_file .= chr(13);	
				
				if($row_truck['id']==679 && 1==2)
				{
					$report_tab.= "
					<tr>
		...
		$export_file .= "".chr(9).
          	...
          	"".chr(9).
          	"".chr(9).
          	"Total Tractor Value: $".money_format('',$truck_value)."".chr(9).
          	"".chr(9).
          	"".chr(9).
          	"".chr(9);
     	$export_file .= chr(13);	
		...
		//Excel spacer
     	...
     	$export_file .= "Substituted Tractors: $replaced_trucks".chr(9);	
     	$export_file .= chr(13);	
		...
		//<br><b>Test Total: $".number_format($mrr_truck_tot,2).":</b>".$mrr_test_trucks."<br>
		$report_tab.= "
			</tbody>
			</table>
		...
		while($row_truck = mysqli_fetch_array($data_trucks)) {
			if($row_truck['replacement_truck_id'] > 0) {
				...
				*/
				...
				//pull value from equipment tracking spreadsheet.... if value is "-1", no record, so use the equipment history value like planned.
				//$vres=mrr_display_equipment_depreciation_final($date_starter,$row_truck['replacement_truck_id'],0);		//date, truck ID, trailer ID
				//if($vres['cur_equip_value'] >=0)			$e_value=	$vres['cur_equip_value'];
				//  //if($vres['cur_unit_value'] >=0)			$e_value=	$vres['cur_unit_value'];
				...
				$export_file .= "$counter".chr(9).
                    	...
                    	"$row_truck[vin]".chr(9).
                    	"$row_truck[name_truck]".chr(9).
                    	"$row_truck_name[name_truck]".chr(9).
                    	"$row_truck[license_plate_no] ".($row_truck['rental'] && $row_truck['insurance_exclude'] ? "(Rental)" : "")."".chr(9).
                    	"$row_truck[leased_from]".chr(9).                    	
                    	"".($row_truck['linedate_aquired'] > 0 ? date("m/d/Y", strtotime($row_truck['linedate_aquired'])) : "")."".chr(9).
                    	"".($row_truck['linedate_returned'] > 0 ? date("m/d/Y", strtotime($row_truck['linedate_returned'])) : "")."".chr(9).
		...
		*/
		...
		//Excel spacer
     	...
     	$export_file .= "ELD and Camera Unit count: ".$pn_unit_cntr."".chr(9);	
     	$export_file .= chr(13);	
     	
		$export_file .= "#".chr(9).
          	"Reference".chr(9).
          	"ELD and Camera Unit Serial Number".chr(9).
          	"Value".chr(9);
     	$export_file .= chr(13);	
		
		$report_tab.= "			
			<div".( $mrr_use_styles > 0 ? "".$stylex."" : " class='section_heading pn_tracker_tab'").">ELD and Camera Unit count: ".$pn_unit_cntr."</div>
			<table".( $mrr_use_styles > 0 ? "".$tablex."" : " class='table_section font_display_section tablesorter pn_tracker_tab'").">
			<thead>
			<tr". ( $mrr_use_styles > 0 ? "".$headerx."" : "").">
				<th><b></b></th>
				<th><b>Reference</b></th>
				<th><b>ELD and Camera Unit Serial Number</b></th>
				<th align='right'><b>Value</b></th>
			</tr>
			</thead>
			<tbody>
				...
				$pn_value += $e_value;
     			...
     			$report_tab.= "
     				<tr>
     					...
     					<td>$row_pn[geotab_device_serial]</td>
     					<td align='right'>".($e_value > 0 ? "$".money_format('', $e_value) : "")."</td>
     				</tr>
     			";
     			...
     			$export_file .= "$pn_counter".chr(9).
          			...
          			"$row_pn[geotab_device_serial]".chr(9).
          			"".($e_value > 0 ? "$".money_format('', $e_value) : "")."".chr(9);
     			$export_file .= chr(13);	
			}
		}		
		...
		$export_file .= "".chr(9).
          	"".chr(9).
          	"(ELD and Camera Unit Value: $".money_format('',$pn_value).")".chr(9).
          	"".chr(9);
     	$export_file .= chr(13);	
		...
		//Excel spacer
     	...
     	$export_file .= "Trailer count: ".mysqli_num_rows($data_trailers)."".chr(9);	
     	$export_file .= chr(13);	
		...
		$report_tab.= "</tbody>
			...
			<div".( $mrr_use_styles > 0 ? "".$mrr_total_head."" : " class='section_heading mrr_total_head'").">(ELD and Camera Unit Value: $".money_format('',$pn_value).")</div>
		...
		while($row_trailer = mysqli_fetch_array($data_trailers)) 
		{
			...
			{											
     			...
     			//pull value from equipment tracking spreadsheet.... if value is "-1", no record, so use the equipment history value like planned.
     			$vres=mrr_display_equipment_depreciation_final($date_starter,0,$row_trailer['id']);		//date, truck ID, trailer ID
     			if($vres['cur_equip_value'] >=0)			$e_value=	$vres['cur_equip_value'];
     			//if($vres['cur_unit_value'] >=0)			$e_value=	$vres['cur_unit_value'];
     			     			
     			//if($row_trailer['id']==1057)			echo "<br>Trailer ".$row_trailer['id']." | Query Value $".$row_trailer['equipment_value']." vs STV Value ".$vres['cur_equip_value'].".<br>SQL=".$vres['sql']."<br>";
			...
			$export_file .= "$counter".chr(9).
               	...
               	"$row_trailer[trailer_owner]".($row_trailer['rental_flag'] > 0 ? " (Rental)" : "")."".chr(9).
               	"$row_trailer[trailer_name]".chr(9).
               	"$row_trailer[license_plate_no]".chr(9).             	
               	"".($row_trailer['linedate_aquired'] > 0 ? date("m/d/Y", strtotime($row_trailer['linedate_aquired'])) : "")."".chr(9).
               	"".($row_trailer['linedate_returned'] > 0 ? date("m/d/Y", strtotime($row_trailer['linedate_returned'])) : "")."".chr(9).
               	"".($e_value > 0 ? "$".money_format('', $e_value) : "")."".chr(9);
          	$export_file .= chr(13);	
			
			$mrr_trailer_tot+=$e_value;
			$mrr_test_trailers.="<br>".$e_value."";			
		}
		//<br><b>Test Total: $".number_format($mrr_trailer_tot,2).":</b>".$mrr_test_trailers."<br>
		$report_tab.= "</tbody>
			</table>
		...
		$export_file .= "".chr(9).
          	...
          	"".chr(9).
          	"".chr(9).
          	"Total Trailer Value: $".money_format('',$trailer_value)."".chr(9).
          	"".chr(9).
          	"".chr(9);
     	$export_file .= chr(13);	
	...
	if($use_excel > 0) 
	{
		...
		$prefix="<br><center><a href=\"/temp/".$excel_filename."\" target='_blank'>Click for Excel Insurance Report</a><center><br>";			
	} 
	echo "".$prefix."";
?>
	</td>
