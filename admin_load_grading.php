<?
$usetitle = "Admin - Load Grading";
$use_title = "Admin - Load Grading";
?>
<? include('header.php') ?>
<?
 	...
 	if(isset($_POST['build_report'])) { 
		...
		<tr>
			<td colspan='13' align='left'>
				<center><span class='section_heading'>Admin - Load Grading</span></center>			
			</td>
			<td colspan='2' align='right'>
				<span class='mrr_link_like_on' onClick='mrr_toggle_graded_good();'>Toggle On Time</span>		
			</td>
		</tr>
		<?	
		$search_date_range = '';
		...
		$mn=mysqli_num_rows($data);
     	...
     	<?
     		...
     		while($row = mysqli_fetch_array($data)) 
     		{
     			...
     			$stop_grader=mrr_select_load_stop_grades("stop_grade_id_".$row['id']."_preplan",$graded,"Ungraded"," onChange='mrr_calc_score_preplan(".$row['id'].");'",1);
     			$stop_fault=mrr_select_load_stop_faults("stop_fault_id_".$row['id']."_preplan",$fault,""," onChange='mrr_calc_score_preplan(".$row['id'].");'");
     			...
     			//get specific fault select boxes................................................................................................................................................................................ 			
     			$selbx_cust="<select name='grade_fault_".$row['id']."_preplan_customer_id' id='grade_fault_".$row['id']."_preplan_customer_id' style='width:".$sel_width1."px;' onChange='mrr_calc_score_preplan(".$row['id'].");'>";
				$selbx_cust.=		"<option value='0'></option>";				
				while($row_customer = mysqli_fetch_array($data_customers)) 
				{ 
					$selbx_cust.=	"<option value='".$row_customer['id']."' ".($row_customer['id'] == $row['grade_fault_customer_id'] ? 'selected' : '').">".$row_customer['name_company']."</option>";
				}				
				$selbx_cust.="</select>";
     			     			
     			$selbx_driver="<select name='grade_fault_".$row['id']."_preplan_driver_id' id='grade_fault_".$row['id']."_preplan_driver_id' style='width:".$sel_width2."px;' onChange='mrr_calc_score_preplan(".$row['id'].");'>";
				$selbx_driver.=	"<option value='0'></option>";
				while($row_driver = mysqli_fetch_array($data_drivers)) 
				{ 
					$selbx_driver.="<option value='".$row_driver['id']."' ".($row_driver['id'] == $row['grade_fault_driver_id'] ? 'selected' : '').">".(!$row_driver['active'] ? '(inactive) ' : '')."".
								$row_driver['name_driver_last'].", ".$row_driver['name_driver_first']."</option>";
				}
				$selbx_driver.="</select>";
						
     			$selbx_truck="<select name='grade_fault_".$row['id']."_preplan_truck_id' id='grade_fault_".$row['id']."_preplan_truck_id' style='width:".$sel_width3."px;' onChange='mrr_calc_score_preplan(".$row['id'].");'>";
				$selbx_truck.=		"<option value='0'></option>";
				while($row_truck = mysqli_fetch_array($data_trucks)) 
				{ 
					$selbx_truck.="<option value='".$row_truck['id']."' ".($row_truck['id'] == $row['grade_fault_truck_id'] ? 'selected' : '').">".(!$row_truck['active'] ? '(inactive) ' : '')."".$row_truck['name_truck']."</option>";
				}
				$selbx_truck.="</select>";
							
     			$selbx_trail="<select name='grade_fault_".$row['id']."_preplan_trailer_id' id='grade_fault_".$row['id']."_preplan_trailer_id' style='width:".$sel_width4."px;' onChange='mrr_calc_score_preplan(".$row['id'].");'>";
				$selbx_trail.=		"<option value='0'></option>";
				while($row_trailer = mysqli_fetch_array($data_trailers)) 
				{ 
					$selbx_trail.="<option value='".$row_trailer['id']."' ".($row_trailer['id'] == $row['grade_fault_trailer_id'] ? 'selected' : '').">".(!$row_trailer['active'] ? '(inactive) ' : '')."".$row_trailer['trailer_name']."</option>";
				}
				$selbx_trail.="</select>";
     			
     			mysqli_data_seek($data_customers, 0);
     			...
     			echo "
     				<tr class='".($counterp % 2 == 1 ? 'odd' : 'even')." all_grades".($graded >=5 ? " good_graded" : "")."'>
     					...
     					<td valign='top'>".$stop_grader."</td>
     					<td valign='top'>".$stop_fault."</td>
     					<td valign='top'><input type='text' name='stop_grade_note_".$row['id']."_preplan' id='stop_grade_note_".$row['id']."_preplan' value=\"".$reason."\" style='width:200px;' onBlur='mrr_calc_score_preplan(".$row['id'].");'></td>
		...
		while($row = mysqli_fetch_array($data)) 
		{
			...
			*/
			...
			$reason=trim($row['stop_grade_note']);
			$fault=$row['grade_fault_id'];
			$stop_grader=mrr_select_load_stop_grades("stop_grade_id_".$row['id']."",$graded,"Ungraded"," onChange='mrr_calc_score(".$row['id'].");'",1);	
			$stop_fault=mrr_select_load_stop_faults("stop_fault_id_".$row['id']."",$fault,""," onChange='mrr_calc_score(".$row['id'].");'");
			...
			//get specific fault select boxes................................................................................................................................................................................ 			
			$selbx_cust="<select name='grade_fault_".$row['id']."_customer_id' id='grade_fault_".$row['id']."_customer_id' style='width:".$sel_width1."px;' onChange='mrr_calc_score(".$row['id'].");'>";
			$selbx_cust.=		"<option value='0'></option>";				
			while($row_customer = mysqli_fetch_array($data_customers)) 
			{ 
				$selbx_cust.=	"<option value='".$row_customer['id']."' ".($row_customer['id'] == $row['grade_fault_customer_id'] ? 'selected' : '').">".$row_customer['name_company']."</option>";
			...
			$selbx_driver="<select name='grade_fault_".$row['id']."_driver_id' id='grade_fault_".$row['id']."_driver_id' style='width:".$sel_width2."px;' onChange='mrr_calc_score(".$row['id'].");'>";
			$selbx_driver.=	"<option value='0'></option>";
			while($row_driver = mysqli_fetch_array($data_drivers)) 
			{ 
				$selbx_driver.="<option value='".$row_driver['id']."' ".($row_driver['id'] == $row['grade_fault_driver_id'] ? 'selected' : '').">".(!$row_driver['active'] ? '(inactive) ' : '')."".
			...
			$selbx_truck="<select name='grade_fault_".$row['id']."_truck_id' id='grade_fault_".$row['id']."_truck_id' style='width:".$sel_width3."px;' onChange='mrr_calc_score(".$row['id'].");'>";
			$selbx_truck.=		"<option value='0'></option>";
			while($row_truck = mysqli_fetch_array($data_trucks)) 
			{ 
				$selbx_truck.="<option value='".$row_truck['id']."' ".($row_truck['id'] == $row['grade_fault_truck_id'] ? 'selected' : '').">".(!$row_truck['active'] ? '(inactive) ' : '')."".$row_truck['name_truck']."</option>";
			...
			$selbx_trail="<select name='grade_fault_".$row['id']."_trailer_id' id='grade_fault_".$row['id']."_trailer_id' style='width:".$sel_width4."px;' onChange='mrr_calc_score(".$row['id'].");'>";
			$selbx_trail.=		"<option value='0'></option>";
			while($row_trailer = mysqli_fetch_array($data_trailers)) 
			{ 
				$selbx_trail.="<option value='".$row_trailer['id']."' ".($row_trailer['id'] == $row['grade_fault_trailer_id'] ? 'selected' : '').">".(!$row_trailer['active'] ? '(inactive) ' : '')."".$row_trailer['trailer_name']."</option>";
			...
			echo "
				<tr class='".($counter % 2 == 1 ? 'odd' : 'even')." all_grades".($graded >= 5 ? " good_graded" : "")."'>
					<td valign='top'>".($row['load_handler_id'] > 0 ? "<a href='manage_load.php?load_id=".$row['load_handler_id']."' target='view_load_".$row['load_handler_id']."'>".$row['load_handler_id']."</a>" : "")."</td>					
					<td valign='top' nowrap>".$row['name_company']."</td>
					...
					<td valign='top'>".$stop_grader."</td>
					<td valign='top'>".$stop_fault."</td>
					<td valign='top'><input type='text' name='stop_grade_note_".$row['id']."' id='stop_grade_note_".$row['id']."' value=\"".$reason."\" style='width:200px;' onBlur='mrr_calc_score(".$row['id'].");'></td>
     	...
     	?>
     	<tr>
     		<td colspan='15'><input type='hidden' name='my_stops' id='my_stops' value='<?= $counter ?>'><hr><input type='hidden' name='my_stops_preplan' id='my_stops_preplan' value='<?= $counterp ?>'></td>
     	</tr>
     	<tr>
     		<td valign='top' colspan='14' align='right'><b>Total Score</b></td>
     		<td valign='top' align='right'><span id='stop_grade_score_tot'></span></td>
     	</tr>
     	<tr>
     		<td valign='top' colspan='14' align='right'><b>Stops Skipped</b></td>
     		<td valign='top' align='right'><span id='stop_grade_score_skip'></span></td>
     	</tr>
     	<tr>
     		<td valign='top' colspan='14' align='right'><b>Delivered Timely</b></td>
     		<td valign='top' align='right'><span id='stop_grade_score_avg' style='color:green; font-weight:bold;'></span></td>
     	</tr>
     	<tr>
     		<td valign='top' colspan='14' align='right'><b>Delivered Late</b></td>
     		<td valign='top' align='right'><span id='stop_grade_score_avg2' style='color:red; font-weight:bold;'></span></td>
     	</tr>
     	<tr>
     		<td colspan='15'><hr><br></td>
     	</tr>
     	<tr>
     		<td valign='top' colspan='14' align='right'><b>Preplan Total Score</b></td>
     		<td valign='top' align='right'><span id='stop_grade_score_tot_preplan'></span></td>
     	</tr>
     	<tr>
     		<td valign='top' colspan='14' align='right'><b>Preplan Stops Skipped</b></td>
     		<td valign='top' align='right'><span id='stop_grade_score_skip_preplan'></span></td>
     	</tr>
     	<tr>
     		<td valign='top' colspan='14' align='right'><b>Preplan Delivered Timely</b></td>
     		<td valign='top' align='right'><span id='stop_grade_score_avg_preplan' style='color:green; font-weight:bold;'></span></td>
     	</tr>
     	<tr>
     		<td valign='top' colspan='14' align='right'><b>Preplan Delivered Late</b></td>
...
<script type='text/javascript'>
	...
	function mrr_update_stop_fault_grade_preplan(stopid)
	{
		...
		custid=$('#grade_fault_'+stopid+'_preplan_customer_id').val();
		driverid=$('#grade_fault_'+stopid+'_preplan_driver_id').val();
		truckid=$('#grade_fault_'+stopid+'_preplan_truck_id').val();
		trailerid=$('#grade_fault_'+stopid+'_preplan_trailer_id').val();
	...
	function mrr_calc_score(update_stop_id)
	{
		...
		for (i=1; i <= v; i++)
		{
			...
			if(stopid==update_stop_id)
			{
				mrr_update_stop_fault_grade(stopid);	
			}
	...
	function mrr_update_stop_fault_grade(stopid)
	{
		...
		custid=$('#grade_fault_'+stopid+'_customer_id').val();
		driverid=$('#grade_fault_'+stopid+'_driver_id').val();
		truckid=$('#grade_fault_'+stopid+'_truck_id').val();
		trailerid=$('#grade_fault_'+stopid+'_trailer_id').val();
	...
	function mrr_toggle_graded_good()
	{
		$('.good_graded').toggle();		
	}
	mrr_calc_score(0);
