<? $usetitle="Sales Report - By Dispatch"; ?>
<? include('header.php') ?>
<?
 	...
 	{	
		...
		$sql = "
			...
			where trucks_log.deleted = 0
				...
				".($_POST['customer_id'] > 0 ? " and trucks_log.customer_id = '".sql_friendly($_POST['customer_id'])."'" : '') ."
	...
	<? 
		...
		while($row = mysqli_fetch_array($data)) 
		{
			...
			if($last_load_id != $row['load_handler_id']) 
			{
				...
				$truck_miles_hr = $row['loaded_miles_hourly'];
				$truck_deadhead_hr = $row['miles_deadhead_hourly'];	
			...
			$use_bill_customer = $row['cost'] + $row['profit'] + $flat_rate_fuel_charge;
			...
			if(isset($_POST['show_dispatches'])) 
			{
				echo "
					<tr class='even'>
						...
						<td align='right' style='color:#aaaaaa'>".number_format($row['loaded_miles_hourly'])."</td>
						<td align='right' style='color:#aaaaaa'>".number_format($row['miles_deadhead'])."</td>
						<td align='right' style='color:#aaaaaa'>".number_format($row['miles_deadhead_hourly'])."</td>						
						<td nowrap>".date("M j, Y", strtotime($row['linedate_pickup_eta']))."</td>
		...
		$sql = "
			select distinct trucks_log.load_handler_id
	...
	<tr>
		<td></td>
		<td colspan='5'><?=number_format($counter)?> dispatch(es),<?=number_format($lcounter)?> Load(s)</td>		
