...
<h2>&nbsp;&nbsp;Dropped Trailer Bill Report</h2>&nbsp;<br>
...
<? if(isset($_POST['build_report'])) { 
	...
	echo "
		<table class='admin_menu2 font_display_section' style='margin:0 10px;width:1300px;text-align:left'>
		<tr>
	...
	while($row = mysqli_fetch_array($data)) 
	{
		...
		*/
		...
		if(($days - $_POST['skip_days']) > 0)
		{				
     		$days_tot+=($days - $_POST['skip_days']);
     		$totdays_tot+=($totdays - $_POST['skip_days']);
     		...
     		echo "
     			<tr>
     				...
     				<td>".$complete_date."</td>
     				<td align='right'>".($days - $_POST['skip_days'])."</td>
     				<td align='right'>".($totdays - $_POST['skip_days'])."</td>
     			</tr>
     		";		//<span style='color:#".($days > 7 ? "CC0000" : "000000").";'>".$days."</span>
     		$days_cntr++;     	
     		
     		$sqlu="update trailers_dropped set invoice_pending='".($days - $_POST['skip_days'])."' where id='".$row['id']."'";
	...
	echo "
			<tr>
				<td><b>".$days_cntr."</b></td>
				<td><b>Total</b></td>
				<td colspan='5'><i>Total is Days dropped within ".date("M j, Y", strtotime($_POST['date_from']))." - ".date("M j, Y", strtotime($_POST['date_to']))." Date Range.</i></td>
