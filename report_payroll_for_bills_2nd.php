...
<?
	...
	function get_master_query($driver_id_filter = 0) {
		...
		if($_POST['employer_id'] > 0)
		{
			...
			//$employ_adder2=" and (drivers2.employer_id = '".sql_friendly($_POST['employer_id'])."')";	//
		}
...
<? if(isset($_POST['build_report']) || $mrr_process > 0) { ?>
	...
	//$data_dexp=get_master_expense_query();
	...
	<?
		...
		while($row = mysqli_fetch_array($data_master)) 
		{
			...
			$total_hours_charge += $row['hours_worked'] * $row['dispatch_charged_per_hour'];
			...
			$mrr_tot_per_hour+=($row['hours_worked'] *$mrr_per_hour_labor);
			
			$mrr_driver_pay_miles += (($row['miles_deadhead'] + $row['miles']) * $mrr_per_mile_labor);
			$mrr_driver_pay_hours += ($row['hours_worked'] *$mrr_per_hour_labor);
			//.............................................................................................................................................................
			...
			if($row['driver2_id'] > 0 && $driver2_employer_id==$temp_employer_id)
			{
				...
				$hour_val+=($row['hours_worked'] * $row['dispatch_charged_per_hour']);				// + $driver_pay_team	
					
				$hour_cntr2+=$row['hours_worked'];
				$hour_val2+=($row['hours_worked'] * $row['driver2_charged_per_hour']);		// + $driver_pay_team	
			...
			if(!isset($_POST['summary_only'])) {
				$mrr_output.= "
					<tr class='".($this_driver_count % 2 == 1 ? 'odd' : 'even')."'>
						...
						<td align='right'>$".number_format(($row['hours_worked'] *$mrr_per_hour_labor ),2)."</td>
			...
			{	//in the loop so add to the loop
				...
				if($mrr_truck_found==0)
				{
     			...
     			$data_dexp=get_master_expense_query($emp_id,$use_driver_only);
		...
		if($employer_used[$i] == 1)
		{
			...
			*/
			...
			if($employer_vendor_bridge > 0)
			{
     		...
     		if(($etot_mile_value + $etot_hour_value + $etot_stop_value + $etot_exps_value)!=0)
     		{
     			echo "
     			<tr>
     				<td colspan='5'><hr></td>
     			...
     			<tr>
     				...
     				<td valign='top' align='right'>".($etot_stops+$etot_exps)."</td>
     				<td valign='top' align='right'>&nbsp;</td>
     				<td valign='top' align='right'>$".number_format(($etot_stop_value+$etot_exps_value),2)."  </td>
     				<td valign='top'>&nbsp;</td>
     			</tr>
     			<tr>
     				<td colspan='5'><hr></td>
     			...
     			<tr>
     				...
     				<td valign='top' align='right'>&nbsp;</td>
     				<td valign='top' align='right'>&nbsp;</td>
     				<td valign='top' align='right'>$".number_format(($etot_mile_value + $etot_hour_value + $etot_stop_value + $etot_exps_value),2)."</td>
