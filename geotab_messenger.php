...
<? 
    ...
    mrr_special_ops_geotab_msg_processing();
	...
	{
		$sql = "
			select geotab_use_id 
			from drivers
			where id='".sql_friendly($_GET['driver_id'])."'
		";
		$data=simple_query($sql);
		if($row=mysqli_fetch_array($data))
		{
			$geotab_use_id=trim($row['geotab_use_id']);			
		}
				
		$truck_id=mrr_find_geotab_drivers_truck_from_elog($geotab_user_id,date("Y-m-d"));
	...
	mrr_trim_old_geotab_datafeed_log_points(14,0,1);	//remove truck_tracking points older than 7 days
	
	mrr_geotab_duel_msg_archiver(7);		//number is days to consider messages new.  If 7, messages are flagged as archived after 7 days. 
		
	$truck_box=mrr_geotab_truck_selector("truck_id",$_POST['truck_id']);
	...
	if($_POST['truck_id'] > 0)
	{
		...
		$truck_drivers2=mrr_find_geotab_truck_drivers_from_elog($_POST['truck_id'],$mydate);
		$truck_drivers3=mrr_find_night_shift_driver_for_truck($_POST['truck_id']);
	}
	...
	{		
		...
		$pick_list=mrr_geotab_truck_grouper('truck_group',$_POST['truck_group']);	
	}
	...
	if(isset($_POST['send_it']))
	{
        ...
        $_POST['emessage']=str_replace(chr(9)," ",$_POST['emessage']);
        $_POST['emessage']=str_replace(chr(10)," ",$_POST['emessage']);
        $_POST['emessage']=str_replace(chr(13)," ",$_POST['emessage']);
        
		if($_POST['truck_id']==0 && $_POST['service_type']=="imessage_send")
		{	//send messages to all trucks
			$tres=mrr_geotab_truck_array();
			$truck_cnt=$tres['num'];
			$truck_arr=$tres['arr'];
			...
			for($t=0;$t < $truck_cnt; $t++)
			{
				...
				//$device_id=trim($devices[$t]);				
				
				$serve_output=mrr_send_geotab_text_message($tid,$_POST['emessage'],$special_msg_flag,0,$_SESSION['user_id']);
				
				if(trim($serve_output)!="0" && trim($serve_output)!="-1" && trim($serve_output)!="-2")	$sent_cnt++;
			...
			$result_message="<span style='color:#00CC00; font-weight:bold;'>Message sent to ".$sent_cnt." GeoTab-tracked trucks.</span>";
			$send_msg="<div class='mrr_maint_recur'>Message sent to ".$sent_cnt." GeoTab-tracked trucks.</div>";
		}
		...
		{	//send messages to all trucks IN GROUP
			...
			for($t=0;$t < $truck_cnt; $t++)
			{
				...
				if(trim($serve_output)!="0" && trim($serve_output)!="-1" && trim($serve_output)!="-2")	{	$sent_cnt++;	$sent_list.=", ".$tname."";		}
			...
			$result_message="<span style='color:#00CC00; font-weight:bold;'>Message sent to ".$sent_cnt." GeoTab-tracked trucks".$sent_list."</span>";
			$send_msg="<div class='mrr_maint_recur'>Message sent to ".$sent_cnt." GeoTab-tracked trucks".$sent_list."</div>";
			
		}
		else
		{
			$serve_output=mrr_send_geotab_text_message($_POST['truck_id'],$_POST['emessage'],$special_msg_flag,0,$_SESSION['user_id']);
				
			if(trim($serve_output)!="0" && trim($serve_output)!="-1" && trim($serve_output)!="-2")	
			{	
				$_POST['emessage']="";
			...
			{
				$send_msg="<div class='alert'>Error: unable to send message.</div>".$serve_output."<br>";	
				$result_message="<span style='color:#CC0000; font-weight:bold;'>Message has been sent.</span>";
			}
			*/
		}
	}
...
?>
<? include('header.php') ?>
<form name='geotab_messenger' action='<?= $SCRIPT_NAME ?>' method='post' onSubmit='return mrr_confirm_message_to_all_drivers();'>
	<input type='hidden' name='date_from' id='date_from' value='<?= date("01/01/Y") ?>'>
...
<tr>
	<td valign='top' colspan='2' width='810'><div class='section_heading'>GeoTab Messenger</div></td>
	<td valign='top' colspan='2' width='790'><div class='section_heading'>Reply Message Viewer</div></td>
</tr>
<tr>
	<td valign='top'>Truck  <?= show_help('geotab_messenger.php','Truck Select') ?></td>
	<td valign='top'>
		<?= $truck_box ?> 
		<div style='display:inline-block; border:0px solid #CC0000; padding-left:5px;'>
			Driver(s): <?= $truck_drivers ?>
			<br>(Alt) Driver(s): <b><?= $truck_drivers2 ?></b>
			<br><span title='Uses this truck between 6:30PM and 6:30AM generally. Day shift is between 6:30AM and 6:30PM.'>Night Shift:</span> <b><?= $truck_drivers3 ?></b>
		</div>
		<br><?= $pick_list ?>
	</td>	
...
<tr>
	<td valign='top'>For Message  <?= show_help('geotab_messenger.php','Message Text') ?> <br><br><br><a href='/peoplenet_messager.php' target='_blank'>Click for "Legacy"<br>PeopleNet<br>Messages</a></td>
	<td valign='top'>
		<?= $canned_messages ?> <input type='button' name='insert_canned_msg' id='insert_canned_msg' value='Add Text' onClick='insert_canned_message();'>  <?= show_help('geotab_messenger.php','Canned Message') ?>	
		<br>
...
<script type='text/javascript'>	
	...
	{			
		$.ajax({
			   type: "POST",
			   url: "ajax.php?cmd=mrr_geotab_messages_mark_read",
	...
	{		
		...
		$.ajax({
			   type: "POST",
			   url: "ajax.php?cmd=mrr_geotab_dispatches_sent",
	...
	{		
		...
		$.ajax({
			   type: "POST",
			   url: "ajax.php?cmd=mrr_geotab_messages_received",
			   data: {"truck_id": $('#truck_id').val(),
			   		"date_from": $('#date_from').val(),
			   		"date_to":  $('#date_to').val(),
			   		"truck_name": $('#truck_name').val(),
	...
	{
		document.geotab_messenger.submit();
	});
